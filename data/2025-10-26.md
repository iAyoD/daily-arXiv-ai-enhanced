...
Motivation: 机器人精确运动学对于工具精确定位至关重要，但非几何因素会引入配置依赖的模型误差，需要开发能够改善整个工作空间精度的校准方法。

Method: 采用局部POE模型在多个配置点进行识别，然后通过傅里叶基函数插值构建全局模型，参数化基于肩关节和肘关节角度。

Result: 在两个6自由度工业机器人上验证，最大定位误差减少超过50%，达到冷喷涂制造所需的亚毫米精度，训练效率显著高于神经网络方法。

Conclusion: 该配置依赖运动学校准框架能有效提高机器人精度，特别适用于存在较大配置依赖误差的机器人，并在双机器人协作任务中展示了实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 非抓取式操作面临未知物理属性和复杂接触交互的挑战，现有CI-MPC方法仅限于简单场景，无法处理多物体复杂接触推理。

Method: 开发了Consensus Complementarity Control Plus (C3+)算法，集成物体扫描、网格重建和硬件执行的完整流程，相比前代C3显著提升求解速度。

Result: 在33个物体测试中达到98%成功率，单物体任务平均耗时约0.5分钟，2/3/4物体任务分别耗时1.6/3.2/5.3分钟。

Conclusion: C3+算法扩展了CI-MPC在多物体复杂接触场景的应用能力，实现了高效实时的非抓取式操作。
...
<details>
  <summary>Details</summary>
Motivation: 传统自主无人机竞速方法受限于预定义赛道布局，缺乏在现实世界中任意状态间飞行的泛化能力。

Method: 使用点质量模型轨迹作为代理奖励来近似最优飞行目标，采用课程学习高效扩展训练过程并实现泛化。

Result: 仿真实验验证了方法的有效性，与非线性模型预测控制相比表现优异，真实世界实验证实了在户外环境中的鲁棒性和泛化能力。

Conclusion: 该学习策略能够在小型ARM单板计算机上运行，实现了任意状态间最小时间飞行的通用化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: Stewart平台并联机构在精密应用中需要微纳米级运动控制，必须比应用精度更高，因此校准至关重要。正向运动学校准过于复杂，而逆运动学校准能更有效地完成此任务。

Method: 通过文献综述分析三种主要校准方法：外部仪器校准、约束条件校准和自校准，重点关注逆运动学基础的PKM校准技术。

Result: 研究发现研究人员主要关注提高平台位置和方向精度，考虑单一或多重误差源，主要误差源是结构性的，部分考虑环境因素，但校准都在无负载条件下进行。

Conclusion: 本研究旨在理解该领域的最新技术水平，并为其他研究人员在特定领域的进一步探索扩展研究范围。
...
<details>
  <summary>Details</summary>
Motivation: 商用和消费级3D打印机多为闭源平台，缺乏研究所需的灵活性和精确参数控制，需要开发专门用于FDM工艺研究的开源测试平台。

Method: 设计制造具有闭环位置反馈、热端和床温控制、环境温湿度监测的FDM平台，配备相机和旋转加热床机制，实现原位摄影测量和几何记录。

Result: 开发出能够精确控制和监测FDM工艺参数的实验平台，通过旋转床机制用最少相机实现打印过程的摄影测量重建。

Conclusion: 该开源平台为FDM工艺研究提供了可重复实验的基础，通过摄影测量技术能够追溯工艺参数与几何缺陷的关联。
...
<details>
  <summary>Details</summary>
Motivation: 传统序列模型忽略运动结构，导致机器人轨迹规划产生无效或低效执行，需要结合运动结构约束的轨迹规划方法。

Method: 使用3网格（位置/内容/时间）表示编码机器人运动，采用约束掩码解码强制相邻网格移动和工作空间边界约束，同时考虑任务图和动作顺序。

Result: 模型与真实轨迹高度一致：步进准确率89.44%，精确率93.32%，召回率89.44%，F1分数90.40%，99.99%路径构造合法；在真实机器人测试中达到97.5%到达成功率、92.5%抓取成功率，60个语言指定任务中86.7%端到端成功率。

Conclusion: 路径结构表示使Transformer能够生成准确、可靠且可解释的机器人轨迹，桥接了基于图的规划和基于序列的学习，为通用操作和仿真到真实迁移提供了实用基础。
...
<details>
  <summary>Details</summary>
Motivation: 磁吸附爬壁机器人在实际应用中面临附着不确定性、部分接触和概率性附着失败等问题，传统控制方法假设完美附着，难以应对这些挑战。

Method: 采用三阶段课程学习：1) 在平地上学习爬行步态；2) 逐渐旋转重力向量并激活吸附模型；3) 注入随机附着失败以训练滑落恢复能力。结合物理基础的吸附模型模拟真实附着行为。

Result: 学习到的策略在模拟中实现高成功率、强附着保持能力和快速脱落恢复。相比假设完美附着的MPC基线，能在间歇性附着失效时维持运动。硬件实验证实了在钢表面上稳健的垂直爬行能力。

Conclusion: 将课程学习与真实吸附建模相结合，为磁吸附爬壁机器人在复杂环境中提供了稳健的仿真到现实迁移框架。
...
<details>
  <summary>Details</summary>
Motivation: 机器人在视觉受限环境（如遮挡、光线不足）中执行操作任务时，需要依赖接触反馈来导航和避障，同时可以利用环境的结构先验来提高效率。

Method: 框架包含三个紧密耦合的组件：(1) 基于关节力矩传感和接触粒子滤波器的接触检测与定位模块；(2) 利用接触观测历史构建部分占据地图并通过学习预测器外推到未探索区域的占据估计模块；(3) 考虑噪声的接触定位和占据预测的规划模块。

Result: 在仿真和真实UR10e机械臂上的评估显示，该框架能可靠完成两个家庭任务（操作厨房水槽下的阀门和从杂乱货架上取物），任务完成时间相比基线最多减少2倍，消融实验确认了各模块的贡献。

Conclusion: 该框架成功整合了接触反馈和结构先验，为视觉受限环境中的机器人操作提供了高效可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决多卫星（超过三个）编队控制中的基本挑战：非完整约束、欠驱动、可扩展性和计算成本，这些挑战随着卫星数量增加而变得更加复杂。

Method: 采用学习辅助的时间积分电流控制方法，设计两轴线圈和基于气浮平台的地面实验装置，引入NODA-MMH算法进行基于模型的功率最优群控制。

Result: 实验验证了平均系统动力学的增强可控性（具有理论保证的误差界限）和分散式电流管理，成功复制了轨道动力学。

Conclusion: 学习辅助的时间积分电流控制为大规模卫星群控制提供了有效的解决方案，特别是在长期编队维持方面具有应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人在连续操作中利用先前观察信息进行高效定位和导航的问题，突破传统方法在上下文长度上的限制。

Method: 使用循环Transformer更新隐式潜在记忆，压缩传感器读数历史为紧凑表示，处理查询图像并预测相对位置。

Result: 模型能有效维护场景表示，导航到剧集开始前观察到的目标，计算效率优于基于观察历史注意力的经典Transformer。

Conclusion: Kinaema在Mem-Nav任务中表现出色，证明了隐式记忆方法在机器人空间感知和导航中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 人类依赖记忆执行任务，但大多数机器人策略缺乏这种能力。直接使用长观察历史计算成本高且脆弱，而随机采样历史会导致信息不相关或冗余。

Method: 分层策略框架：高层策略训练选择并跟踪历史相关关键帧，使用选中的关键帧和最新帧生成文本指令给底层VLA模型执行。基于Qwen2.5-VL-7B-Instruct和π0.5模型微调，使用带最小语言标注的演示数据。

Result: MemER在三个需要分钟级记忆的真实世界长时程机器人操作任务中优于现有方法。

Conclusion: 该框架兼容现有VLA模型，能高效处理长时程依赖关系，成功赋予机器人策略记忆能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端方法在领域内表现良好，但在领域偏移（如天气和光照变化）下的鲁棒性仍是关键挑战。本文旨在不依赖额外数据的情况下提升跨领域性能。

Method: 整合视觉基础模型与基于扩散的规划，在CARLA中训练标准设置，并以零样本方式迁移到更恶劣的设置。

Result: 在所有测试的分布外场景中，模型持续实现90%以上的停车成功率，消融研究证实网络架构和算法设计显著提升了跨领域性能。

Conclusion: 在从真实世界停车场重建的3D高斯泼溅环境中测试显示出有前景的模拟到真实迁移能力。
...
<details>
  <summary>Details</summary>
Motivation: 模块化可重构机器人适合空间任务，但形态组合爆炸阻碍统一控制。

Method: 分散式强化学习方案：轮式模块使用SAC进行运动控制，7自由度肢体使用PPO进行转向和操作。

Result: 转向策略平均绝对误差3.63度；操作策略成功率84.6%；轮式策略平均扭矩降低95.4%且成功率99.6%。月球模拟测试验证零样本集成。

Conclusion: 系统在策略执行中平滑切换同步、并行和顺序模式，无空闲状态或控制冲突，表明该方法可扩展、可重用且鲁棒。
...
Method: 利用NDF隐式表示目标接触几何，训练深度强化学习策略使用触觉反馈来优化抓取，该策略以神经描述符为条件，不需要显式指定接触类型。

Result: 在模拟和真实世界操作任务（如孔轴装配和瓶盖开启）中验证，无需额外微调即可实现零样本迁移，显著提高了抓取精度和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前点云方法难以捕捉细粒度细节，而RGB方法缺乏几何感知能力，这限制了机器人操作系统的精度和泛化能力。需要一种能够结合几何结构和语义信息的互补感知方法。

Method: 提出PointMapPolicy方法，将点云条件化为结构化网格而不进行降采样，使用xLSTM作为骨干网络，有效融合点图与RGB数据，并利用成熟的计算机视觉技术处理3D数据。
...
<details>
  <summary>Details</summary>
Motivation: 解决水下机器人手臂遥操作中视觉和触觉反馈整合不足的问题，提高操作的稳定性和准确性。

Method: 使用混合现实头戴显示器(MR-HMD)显示颜色和长度编码的扭矩条作为反应扭矩指示器(RTI)，结合双边控制实现视觉和触觉反馈的集成。

Result: 与双边控制基线相比，MR-UBi显著提高了抓取扭矩控制精度，增加了在最佳扭矩范围内的时间，减少了低和高抓取扭矩范围，在提升和拾取-放置任务中表现更好。主观评估显示更高的可用性和更低的工作负荷。
...
<details>
  <summary>Details</summary>
Motivation: 当移动零件比移动工具更容易时，需要一种能够规划基于固定TCP和零件加工路径的机器人轨迹的方法。

Method: 使用B样条表示机器人路径，确保路径连续性和平滑轨迹。在计算机器人轨迹时考虑规定的方向和TCP的给定速度。

Result: 该方法在真实系统上使用工业机器人移动任意定义的零件进行了验证。

Conclusion: 提出的方法能够有效规划基于固定TCP和零件加工路径的平滑机器人轨迹，适用于移动零件比移动工具更容易的工业场景。
...
<details>
  <summary>Details</summary>
Motivation: 在GNSS拒止环境中，单个机器人携带所有传感器会增加尺寸、重量和功耗需求。将传感器分布在多个机器人上可提高部署性，但面临融合异步多模态数据的挑战。

Method: 使用因子图框架以松耦合方式融合异步VIO、LIO和3D机器人间检测。提出基于插值的因子处理未同步测量，基于近似扫描匹配Hessian评估LIO退化，基于Wasserstein距离对VIO输出进行加权。

Result: 在真实世界数据上广泛评估，使用异构UGV和UAV团队，显示该方法在各种传感器退化情况下显著提高了定位精度。

Conclusion: 该方法能够适应变化条件，利用可靠数据辅助受传感器退化影响的机器人，在多机器人协同定位中表现出显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人操作任务在未知动力学（如负载不确定性）下的控制问题，需要主动探索和在线参数适应来实现精确的基于模型控制。

Method: 将问题构建为双重控制问题，预定义反馈策略结构包含显式适应机制。提出两种参考轨迹生成方法：一种在鲁棒最优控制中嵌入参数不确定性，另一种最小化最优性损失。

Result: 两种方法都自然地考虑了Fisher信息，同时追求最优任务执行。在拾取-放置操作任务中展示了方法的有效性，实现了更快更准确的任务性能和系统辨识。
...
<details>
  <summary>Details</summary>
Motivation: 工业机器人执行拾取-放置任务时长期运行，能耗优化至关重要。串联弹性驱动器(SEA)的弹性特性提供了节能潜力，通过激发固有振动来减少能量消耗。

Method: 推导SEA驱动并联机器人的动力学模型，建立能量最小化的最优控制问题，同时优化操作轨迹和SEA刚度参数，并在两个并联机器人应用上进行验证。

Result: 研究结果证实了该方法的有效性，通过优化轨迹和刚度参数能够显著降低拾取-放置任务的能量消耗。

Conclusion: 利用SEA的弹性特性和固有振动优化轨迹和刚度参数，是降低并联机器人拾取-放置任务能耗的有效方法，为机器人设计提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 传统时间最优路径跟踪方法在最小化旅行时间时，在零路径速度处会产生奇异性，导致计算复杂且难以生成平滑轨迹。

Method: 采用最大化路径速度的方法，将问题离散化后优化变量呈线性关系，实现数值高效计算。

Result: 该方法能够有效避免奇异性问题，实现平滑轨迹规划，同时保持较低的计算复杂度。

Conclusion: 提出的最大化路径速度方法为时间最优路径跟踪提供了一种计算高效且能生成平滑轨迹的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 由于法律限制和机构所有权问题，真实结构坍塌事故中的废墟内部空间数据难以公开获取，而训练场地也不愿公开其专有信息。

Method: 使用Unity开发多操作系统支持的模拟器，采用基于物理的方法随机生成废墟堆，能够快速迭代不同模拟场景并保留真实地面信息。

Result: 应用最先进的结构运动算法，展示了在模拟废墟内部恶劣视觉条件下感知性能的退化情况。

Conclusion: RubbleSim为废墟内部空间探索提供了一个可重构的、开源的仿真解决方案，有助于研究在挑战性视觉条件下的感知性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态轨迹和固定对象类别，忽略了现实世界中对动态环境的持续适应需求。

Method: C-Nav框架包含双路径抗遗忘机制（特征蒸馏和特征回放）和自适应采样策略，确保表示一致性和策略一致性。

Result: 在多种模型架构上的实验表明，C-Nav优于现有方法，即使与保留完整轨迹的基线相比也表现更优，同时显著降低内存需求。

Conclusion: C-Nav为持续对象导航提供了有效的解决方案，在性能和内存效率方面均表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决无模型强化学习策略收敛到单一步态导致性能次优，以及模型预测控制缺乏环境适应性的问题。

Method: 使用MPPI算法联合优化动作和步态变量，结合Dreamer模块学习奖励函数，促进速度跟踪、能效、稳定性和平滑过渡。

Result: 在Unitree Go1仿真中，不同目标速度下能耗平均降低36.48%，同时保持准确跟踪和自适应步态。

Conclusion: 该框架成功实现了四足机器人的实时步态自适应，在保持性能的同时显著降低能耗。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习推动了机器人技术的进步，但模拟环境与现实世界之间存在差异（现实差距），这阻碍了系统从模拟到现实的成功转移。理解现实差距的根本原因和解决方案是当前机器人学中最紧迫的挑战之一。

Method: 通过领域随机化、现实到模拟转移、状态和动作抽象、模拟-现实协同训练等技术来克服现实差距。

Result: 近年来在模拟到现实转移方面取得了有希望的结果，包括在运动、导航和操作等各种平台上的应用。

Conclusion: 论文提供了模拟到现实转移领域的全面概述，重点分析了现实差距的原因、解决方案和评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够实现逼真渲染和物理模拟的机器人操作模拟器，以支持可重复的策略评估和sim2real训练，减少对真实机器人的依赖。

Method: 提出GSDF（高斯场景描述文件）资产格式，结合高斯-网格表示与机器人URDF和其他对象，通过流线化重建流程构建包含3种机器人形态和40多个对象的数据库。

Result: 展示了多个应用：零样本sim2real像素到动作策略学习、自动化高质量DAgger数据收集、可重复基准测试、虚拟遥操作数据收集和零样本sim2real视觉强化学习。

Conclusion: GSWorld为机器人操作提供了一个强大且逼真的模拟环境，支持多种sim2real应用，推动了无需真实机器人的策略开发与评估。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人导航中学习跨环境通用策略与适应特定机器人物理约束之间的基本挑战，如四足机器人能上楼梯但轮式机器人不能。

Method: 分层VLA架构：高层通用规划器从多样化开放世界数据学习，底层专用可达性模型在低成本模拟中学习机器人物理约束；通过图像空间候选路径提案和可达性评估重排实现解耦。

Result: 在室内和复杂室外导航中比最先进的基于模型和端到端学习方法获得更高成功率；支持轮式和腿式机器人跨具身导航；通过拒绝物理不可行计划使单机器人可靠性提高3倍。

Conclusion: 分层设计是实现跨具身导航的关键，专用模型对具身接地至关重要，使单一高层规划器能部署到物理特性不同的轮式和腿式机器人上。
