...
Motivation: 研究机器人在高风险场景中的干预潜力，特别是通过分散和阻碍校园枪手来增强安全性。

Method: 使用虚拟现实模拟，150名大学生扮演校园枪手角色，机器人预测枪手移动并战略定位干扰。操纵机器人接近策略（激进vs被动）和分散方法（无提示vs中等vs高分散）。

Result: 激进且高分散的机器人相对于无机器人对照组，受害者数量减少了46.6%。
...
Method: 构建了RoboChallenge在线评估系统，采用Table30作为初始基准，对最新VLA模型进行系统评估。

Result: 成功开发了在线评估系统，并对当前最先进的VLA模型进行了全面调查。

Conclusion: RoboChallenge系统为机器人控制算法的大规模评估提供了有效解决方案，支持可扩展和可复现的测试。
...
Result: 在真实世界实验中，人形机器人成功实现了敏捷、自主且自然的快速移动球拦截，并展示了在球逃逸和抓取等任务中的泛化能力。

Conclusion: 为机器人与移动物体之间的高度动态交互提供了一个实用且可扩展的解决方案，推动了更自适应和逼真的机器人行为发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决多机器人在m维流形上导航时，由于流形特性导致GVF传播项中最后m个条目强耦合的问题，以及非欧几里得度量下灵活排序协调的设计挑战。

Method: 首先识别可行的辅助向量解耦传播项，然后重新设计协调GVF算法，将m个流形参数视为额外的虚拟坐标，并通过共享虚拟坐标实现灵活的运动协调。

Result: 通过不同初始位置、高维流形和机器人故障的广泛仿真，展示了算法的灵活性、适应性和鲁棒性。

Conclusion: 所提出的算法成功解决了多机器人在流形上的导航和灵活排序协调问题，具有消除奇点、全局收敛和避免复杂计算的优势。
...
Method: R2BC方法允许人类操作员每次遥控一个智能体，通过顺序的单智能体演示逐步教导多智能体行为。

Result: R2BC方法在四个多智能体模拟任务中匹配甚至超越了基于特权同步演示的oracle行为克隆方法，并在两个物理机器人任务中成功部署。

Conclusion: R2BC是一种有效的多机器人系统训练方法，通过简单的单智能体演示就能实现高性能的多智能体协作。
...
<details>
  <summary>Details</summary>
Motivation: 水果采摘主要是劳动密集型过程，现有刚性或真空驱动夹爪需要复杂机械设计或高能耗，而现有包裹式水果采摘夹爪缺乏对不同尺寸水果的适应性。

Method: 采用3D打印TPU材料制作带有集成钢丝的口袋结构，通过伺服驱动线缆控制实现收缩，均匀分布压力以减少损伤，并具有可调夹持力。

Result: 对于夹爪有效尺寸范围内的番茄，采摘实现了0%即时损伤率和5天后小于9%的瘀伤率。

Conclusion: 该夹爪设计适合水果采摘应用，具有轻量化、低成本和低损伤的特点。
...
<details>
  <summary>Details</summary>
Motivation: 探讨机器人学习中数据集的价值，特别关注接触数据的重要性，因为接触包含了对机器人学习至关重要的信息。

Method: 推导接触感知的目标函数，从位姿和接触数据中学习物体动力学和形状，使用接触感知的Fisher信息度量来对接触数据进行排序和筛选。

Result: 基于信息度量的数据筛选能改进学习任务并使学习过程确定性化，较少但信息丰富的数据比更多数据更能加速学习，特别是根据接触交互的不同。

Conclusion: 提出的度量方法能为基于接触的机器人学习提供初步的数据筛选指导，强调数据质量而非数量对学习效果的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 从大规模人类演示中学习对机器人训练有效，但收集双手机器人移动操作数据成本高昂且耗时。现有方法无法解决移动环境中的基座放置和相机定位挑战。

Method: 将数据生成建模为约束优化问题，强制执行可达性等硬约束，同时平衡导航过程中的可见性等软约束。

Result: 在四个多步骤双手机器人移动操作任务上，MoMaGen生成的数据集比现有方法更多样化，仅需单个源演示就能训练成功策略，仅需40个真实世界演示即可在物理硬件上部署。

Conclusion: MoMaGen为多步骤双手机器人移动操作提供了一个原则性的数据生成框架，显著减少了真实世界数据需求，为未来方法奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法面临两个挑战：当不使用生成推理作为条件时语言可控性有限，或者当引入推理时推理延迟显著。需要一种既能保持通用智能又能高效执行的方法。

Method: 采用混合变换器架构，包含通用专家（预训练VLM）和领域专家（共享知识的第二个变换器）。领域专家生成领域特定的快速推理（如机器人运动分解），动作专家基于分解的运动指令学习多样化行为。

Result: 在自然语言处理基准、机器人仿真环境和真实世界实验中，MoTVLA在快速-慢速推理和操作任务性能方面均表现出优越性。

Conclusion: MoTVLA成功整合了快速-慢速统一推理与行为策略学习，显著提升了语言可控性和策略执行效率，为机器人学习中的开放世界泛化提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 实现高质量3D重建需要从不同视角捕获关键点图像，而实时3D重建算法的发展使得能够在飞行过程中渲染演化地图，为无人机运动提供即时反馈

Method: 结合QP角度感知覆盖控制器协调无人机运动，使用NeuralRecon算法实时处理图像生成3D网格，将网格变化作为重建不确定性指标来更新覆盖控制的重要性指数

Result: 仿真和实验验证了Coverage-Recon的有效性，定性和定量分析表明相比传统方法，集成在线地图反馈能产生更完整准确的3D重建

Conclusion: Coverage-Recon通过整合在线地图反馈显著提升了多无人机协同3D重建的质量和完整性
...
<details>
  <summary>Details</summary>
Motivation: 现有的感知强化学习控制器要么施加步态先验限制动作空间、增加策略优化偏差并降低跨机器人形态的适应性，要么"盲操作"难以预测后腿地形且对噪声脆弱。

Method: PGTT通过奖励塑造实施步态结构，使用三次Hermite样条编码每腿相位，根据局部高度图统计调整摆动高度，添加摆动相位接触惩罚，策略直接在关节空间中操作。

Result: 在MuJoCo中训练，PGTT在推力扰动下成功率中位数提高7.5%，在离散障碍物上提高9%，收敛速度比强端到端基线快约2倍，并在Unitree Go2和ANYmal-C上验证。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统验证需要在测试保真度、成本和可扩展性之间权衡，而小型化硬件在环平台缺乏支持严格定量分析的系统框架，限制了其作为科学评估工具的价值。

Method: 提出了三阶段测试流程面向SOTIF标准，设计了以统一时空测量核心为中心的硬件在环平台，确保物理运动和系统时序的一致可追溯量化。

Result: 平台验证显示空间精度达到10.27毫米RMSE，闭环延迟基线约45毫秒。Autoware案例研究发现注入40毫秒延迟时出现关键性能悬崖。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法大多依赖关节中心位置且为离线处理，缺乏对传感器姿态、人体姿态和个体差异的鲁棒性，无法满足工业场景实时动作识别的需求。

Method: 构建完整实时人体关节运动学估计流程，引入生物力学先验知识（关节角度），结合时间平滑Transformer网络进行动作识别。

Result: 在包含11名受试者的新数据集上评估，准确率达到88%，优于最佳基线模型，对非正面摄像头的受试者表现出良好泛化能力，并通过在线机器人交互实验验证实用性。

Conclusion: 基于关节角度的方法在实时性、鲁棒性和跨环境泛化方面具有优势，为工业场景实时动作识别提供了有效解决方案。
...
Conclusion: 该MPC公式填补了非完整系统和非凸约束下输出跟踪MPC的理论空白，为自主移动机器人运动规划提供了可靠的理论基础
...
<details>
  <summary>Details</summary>
Motivation: 解决传统仿真到现实方法依赖大量离线仿真和模型无关策略优化的局限性，直接在真实世界交互中学习动态模型。

Method: 从实时交互数据构建动态模型，基于学习到的动态模型指导策略更新，采用在线学习分析推导性能保证。

Result: 在液压挖掘臂和软体机器人臂上验证，相比模型无关方法具有更强样本效率，数小时内达到相当性能，对动态变化具有鲁棒适应性。

Conclusion: 该方法为挑战性控制任务提供了高效可靠的机器人上学习方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的物体导航系统严重依赖云端大模型，而直接切换到本地小模型会导致成功率显著下降和规划延迟过高，无法在本地设备上部署。

Method: 提出语义感知内存检索来修剪导航图中的冗余信息，帮助小模型更好理解环境；使用离散内存缓存和基于注意力的内存聚类来高效保存和重用KV缓存。

Result: 在HM3D基准测试中，相比GPT-4基线方法成功率提升11.1%，实时延迟降低6.7倍，端到端延迟降低4.7倍。

Conclusion: EfficientNav成功实现了设备端高效的LLM零样本物体导航，在保持高成功率的同时显著降低了延迟，为本地部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统无人机在抓取和栖息功能上存在控制耦合、结构复杂等问题，需要一种能集成多种功能且控制解耦的新型无人机设计。

Method: 开发了Flexbee的动力学模型，通过等效力矩模型的线性化解决非线性耦合问题，采用分层控制策略设计两种操作模式的控制器。

Result: 通过飞行、抓取和栖息实验验证了Flexbee的运动学能力和控制策略的有效性。

Conclusion: Flexbee成功实现了飞行、抓取和栖息功能的集成，软矢量推进喷嘴设计提供了控制解耦和结构复用等优势。
...
<details>
  <summary>Details</summary>
Motivation: 腿式机器人能够比轮式探测器穿越更困难的地形，如跳过地面裂缝或在崎岖地形中移动，从而扩展未来行星探测任务的范围和能力。

Method: 在武尔卡诺火山（月球和火星表面的高保真模拟环境）进行实地实验，测试新开发的自适应最优控制算法，包括技术方法、测试计划、软件架构和现场部署策略。

Result: 成功验证了四足机器人自适应最优控制算法在模拟行星环境中的性能。

Conclusion: 腿式机器人结合先进控制算法，能够显著增强行星探测任务在困难地形中的移动能力，提高科学回报。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在广泛任务中表现出色，但难以适应动态现实场景而无需完全重新训练

Method: 应用持续学习和组合性原则来增强基础模型的适应性

Result: 提出了使基础模型更灵活适应现实世界场景的方法论

Conclusion: 持续学习和组合性可以解决基础模型在动态环境中的适应性问题
...
<details>
  <summary>Details</summary>
Motivation: 传统CBF方法在处理复杂形状不安全区域时往往需要保守近似，这会限制系统的控制性能。保守约束只有在阻止我们实现期望控制时才成为问题。

Method: 通过优化CBF和安全控制的组合，而不是先选择CBF再选择安全控制。提供了CBF族的光滑参数化方法，并在带加速度约束的双积分器动态系统中验证。

Result: 该方法能够在保证安全性的同时，使控制动作尽可能接近期望控制，解决了传统保守CBF方法的限制问题。

Conclusion: 提出的最小限制超平面CBF方法能够有效平衡安全性和控制性能，适用于处理任意形状静态和动态障碍物的复杂场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统腿式移动在松散和倾斜表面上可能能量效率低且对机器人危险，需要开发自适应移动策略

Method: 结合Isaac Sim中的物理仿真和ANSYS-Rocky中的粒子交互验证

Result: 识别了行走和滑行CoT曲线的交叉点，定义了触发策略转换的阈值条件

Conclusion: 这是朝着行星腿式漫游车自适应移动策略迈出的初步步骤
...
<details>
  <summary>Details</summary>
Motivation: 当前语义场景表示方法缺乏空间特征与动态事件之间的连接，限制了机器人对环境理解的丰富性。

Method: 构建事件接地图框架，将事件交互与场景的空间特征进行接地连接。

Result: 使用真实机器人数据的实验表明，EGG能够检索相关信息并准确响应人类关于环境和事件的查询。

Conclusion: EGG框架能够有效连接空间特征和动态事件，提升机器人的环境理解和交互能力，相关代码和数据集已开源。
...
Method: 基于激光雷达教学重复的精确路径跟踪，开发分布式模型预测控制器，使用共享地图进行车辆间相对定位。

Result: 分布式MPC与集中式MPC性能相当，在双车和三车编队中实时运行，各种条件下保持目标间距20厘米内的精度。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人在真实室内环境中运行时，如何将传感器检测结果整合为持久、可查询的语义对象地图的问题。

Method: 系统整合距离几何与相机检测，在帧内合并同位置检测，并在帧间将重复检测关联到持久对象实例。

Result: 在机器人测试中，对象层在不同视角变化下保持稳定，输出紧凑的对象层，可与占据地图集成并被规划器读取。

Conclusion: 该系统能够创建稳定、持久的语义对象地图，支持机器人导航和规划任务。
...
<details>
  <summary>Details</summary>
Motivation: 传统HJ可达性方法受维度诅咒限制，而物理信息深度学习虽然能克服这一问题，但存在收敛慢和不准确的问题。现有方法仅限于单人问题和简单博弈，需要扩展到更复杂的双人零和博弈场景。

Method: MADR框架结合模型预测控制（MPC）引导和对抗性深度学习方法，通过增强自监督过程的正则监督来近似双人零和微分博弈的价值函数，同时为双方玩家生成最优策略。

Result: 在多个高维模拟和真实机器人代理上测试，MADR显著优于现有最先进方法，在仿真中表现优异，在硬件实验中产生令人印象深刻的结果。

Conclusion: MADR为双人零和微分博弈提供了一个通用且鲁棒的解决方案框架，成功克服了高维HJ可达性问题的挑战，并在实际应用中展现出优越性能。
