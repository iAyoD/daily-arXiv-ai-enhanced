...
Method: 引入基本术语和基本逼近定理，使用多种算子（SK、高斯、双边、阈值小波）进行误差测量，分析MSE、SI、SSI、SMPI、ENL等数学参数，通过数值实验和表格对比验证。

Result: 不同算子在图像处理中表现各异，某些算子对特定图像特征效果较好，而其他算子可能不适用，验证了基本逼近定理的正确性。

Conclusion: 由于图像的非理想特性，各种算子在研究图像不同特征时各有其重要性，需要根据具体应用场景选择合适的算子。
...
<details>
  <summary>Details</summary>
Motivation: 智能制造环境中需要准确实时的工人动作识别，但传统的集中式数据集在隐私敏感场景中不实用。

Method: 使用自定义的上身手势骨骼数据集，采用修改的FastPose模型，比较LSTM和Transformer编码器两种时序模型，在四种训练范式下进行评估。

Result: 在全局测试集上，FL Transformer比集中式训练提升12.4%，FedEnsemble提升16.3%。在未见客户端上，FL和FedEnsemble分别超过集中式准确率52.6%和58.3%。

Conclusion: 联邦学习不仅能保护隐私，还能显著提升跨用户泛化能力，是异构工业环境中可扩展、隐私意识的HAR实用解决方案。
...
Method: 提出可扩展的强化学习框架LENS，使用统一的强化学习奖励（句子级、边界框级和分割级线索），联合优化推理过程和分割质量。
...
<details>
  <summary>Details</summary>
Motivation: 计算机视觉中物体尺度变化是基本挑战，同类物体可能有不同尺寸，且受相机距离影响，这些变化是局部的，需要有效处理尺度变化
...
Conclusion: DEC方法有效解决了局部尺度变化问题，能显著提升现有模型的性能和尺度一致性，具有很好的实用价值
...
<details>
  <summary>Details</summary>
Motivation: 视频异常检测在计算机视觉中具有重要意义，但该领域在不同领域和学习范式之间存在碎片化现象，需要系统性的整理和综述来推动理论理解和实际应用。

Method: 采用系统性综述方法，从监督级别(全监督、弱监督、无监督)、自适应学习方法(在线学习、主动学习、持续学习)以及三大应用类别(以人为中心、以车辆为中心、以环境为中心)等多个维度对VAD文献进行组织和分析。

Result: 识别了当前方法的基本贡献和局限性，为研究社区提供了结构化的基础框架，有助于推动VAD系统的理论发展和实际部署。

Conclusion: 该综述为研究人员提供了有价值的参考，同时指出了异常检测领域更广泛的开放挑战，包括基础研究问题和实际部署中的障碍，有助于指导未来研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络在处理复杂数据结构时存在局限，需要探索能够更好建模关系数据的新范式。GCN与Voronoi图的结合为图像分类提供了新的解决方案。

Method: 使用基于图的图像表示方法，将像素或区域视为图顶点，通过Delaunay三角剖分进行简化，并提出了归一化Voronoi图卷积网络(NVGCN)模型。

Result: 在多个基准数据集上实现了预处理时间和分类准确率的显著提升，特别是在复杂场景和细粒度分类任务中超越了现有最先进模型，并通过交叉验证证实了结果的有效性。

Conclusion: GCN与Voronoi图的整合为图像分类任务提供了新的发展方向，同时为计算机视觉和其他非结构化数据领域的图基学习范式开辟了新途径。
...
Method: 引入两个新任务：重建图像顺序和文本顺序；提出定向令牌方法来捕获视觉和文本知识；引入图像到响应引导损失来改进视觉理解。

Result: 在学术任务导向和指令跟随的多模态模型基准测试中持续实现最先进的性能。

Conclusion: 该方法通过排序重建任务有效提升了多模态模型的跨模态对齐能力和整体性能，为改进模型鲁棒性和泛化性提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医疗应用中需要可靠的不确定性量化，而数据增帽策略与符合预测统计保障的交互作用尚不明确

Method: 使用DDR数据集，评估ResNet-50和CoaT Transformer两种网络结构在5种增帽策略下的表现：无增帽、标准几何变换、CLAHE、Mixup和CutMix

Result: 样本混合策略（Mixup和CutMix）既提高预测准确性又提供更可靠高效的不确定性估计，而CLAHE等方法可能对模型确定性产生负面影响

Conclusion: 需要在数据增帽策略设计时考虑下游不确定性量化，以建立真正可信的医疗图像AI系统
...
<details>
  <summary>Details</summary>
Motivation: 尽管CBCT扫描在牙科诊断和治疗规划中日益重要，但生成具有精细控制能力的解剖学真实扫描图像仍然是一个挑战，需要能够实现局部牙齿修改而不需要重新扫描的技术。

Method: 采用小波去噪扩散模型，结合FiLM条件调节和掩码损失函数，专注于相关解剖结构的学习，通过牙齿级二元属性进行精确控制。

Result: 模型在保真度和泛化能力方面表现优异，FID分数低，修复性能强，SSIM值超过0.91，即使在未见过的扫描图像上也能保持良好性能。

Conclusion: 该方法能够实现真实、局部的牙列修改，为外科规划、患者沟通和牙科AI工作流中的针对性数据增强开辟了新机会。
...
Method: 使用自监督对比学习提取场景特定的3D实例特征场，引入带有两个可学习码本的跨注意力模块来编码视图无关的语义嵌入，避免每个高斯的高维特征学习以减少内存消耗

Result: 在真实世界数据集上的广泛实验表明，GALA在2D和3D开放词汇查询方面都表现出卓越的性能

Conclusion: GALA框架通过创新的跨注意力设计和自监督学习，成功实现了高效的开放词汇3D场景理解，在保持性能的同时显著降低了内存需求
...
Method: 提出对比条件推理（CCI）框架，显式建模图像嵌入、类别标签和理由之间的概率关系，无需训练即可有效基于理由预测准确物体类别。

Result: 在多理由可解释物体识别基准上达到最先进结果，包括强大的零样本性能，在分类准确性和理由质量方面都设立了新标准。
...
<details>
  <summary>Details</summary>
Motivation: 在急性缺血性卒中血管内取栓治疗中，准确检测血管闭塞至关重要。数字减影血管造影序列的解释因解剖结构复杂和时间紧迫而具有挑战性，需要自动化解决方案。

Method: 提出OccluNet模型，整合YOLOX单阶段目标检测器和基于transformer的时序注意力机制。探索了两种时空变体：纯时序注意力和分时空注意力。与基于单帧或最小强度投影的YOLOv11基线模型进行比较。

Result: 在MR CLEAN Registry的DSA图像评估中，模型能够捕捉时序一致特征，达到89.02%的精确率和74.87%的召回率。OccluNet显著优于基线模型，两种注意力变体性能相似。
...
Method: 端到端行为克隆训练，结合人工标注的游戏演示和未标注的公共视频数据，通过逆动力学模型推断动作，使用仅解码器的transformer架构处理大动作空间

Result: 在简单Roblox和经典MS-DOS游戏中表现出有竞争力的游戏能力，进行了未标注数据的消融实验，展示了模型的有效性

Conclusion: 该模型为达到专家级文本条件控制奠定了基础，需要进一步的扩展和评估步骤来实现更高水平的游戏表现
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶视频生成方法主要关注RGB视频生成，缺乏多模态生成能力，而多模态数据对于完整的城市场景理解至关重要
...
Conclusion: 提出的统一框架成功解决了自动驾驶中多模态视频生成的挑战，为多模态城市场景理解提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: IoU-based损失函数在小物体检测中存在梯度更新不足的问题，因为小物体的梯度极其平缓，导致训练过程中小物体的学习效率低下。

Method: 利用类别间的空间关系（如车牌与汽车的附着关系），当预测的小物体边界框不在其相关大物体内部时，添加与重叠面积成反比的惩罚损失，通过大物体来指导小物体的预测。

Result: 在YOLOv12-T和UAV-DETR上分别实现了10.3%和1.6%的mAP50提升，无需额外超参数调优。

Conclusion: 提出的ICR损失函数可以轻松集成到现有的IoU-based损失中，有效提升小物体检测性能，特别是在多目标检测任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 手语识别模型因训练数据不足而遇到性能限制，需要一种新的方法来解决这个挑战。

Method: 提出了基于CMLPe的轻量级手语生成模型，结合合成数据预训练方法，使用Mamba-SL和Transformer-SL分类器进行识别。

Result: 在LSFB和DiSPLaY数据集上达到了新的state-of-the-art结果，合成数据预训练在某些情况下超过传统数据增帿方法，并且与其他方法补充使用时效果更好。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Taxol效果检测方法需要专业设备和复杂的样品处理，成本高且不适合高速量分析。深度学习在医学图像分析中显示了强大潜力，但缺乏相关公开数据集。

Method: 收集了C6脑研细胞在不同Taxol浓度处理下的微视镜图像数据集，并提出ResAttention-KNN基线模型，结合ResNet-50、卷积注意力模块和k近邻分类器。

Result: 建立了公开数据集和基线模型，为自动化细胞形态分析提供了标准化的评估基准，支持可复现性研究。

Conclusion: 该研究填补了Taxol细胞浓度分析领域的数据空白，通过注意力机制和非参数分类提高了模型的稳健性和可解释性，为视觉基图像生物医学分析提供了重要资源。
...
Method: 提出HRC-Pose深度框架，使用对比学习学习点云表示，解耦旋转和平移分量分别编码，基于6D姿态感知的层次排序方案设计对比学习策略。

Result: 在REAL275和CAMERA25基准测试中一致优于现有深度方法，能够学习连续特征空间，且实时运行。
...
Method: 采用两种预训练策略将音频分解为独立成分并将视频量化为视觉标记组合；提出情感锚点(EA)表示将情感信息整合到视觉标记中；使用自回归变换器建模视觉标记的全局分布并预测合成视频的索引序列。

Result: 在MEAD数据集上进行实验，该方法在多个情感音频条件下控制视频情感，定性和定量实验均显示出优越性能。

Conclusion: 所提出的离散化方法能够有效实现情感可控的说话人脸生成，在保持身份特征的同时成功整合情感信息。
...
Result: 提出的方法在新引入的数据集和多个公共基准测试中都取得了稳健的性能，在MOT17和MOT20测试集上分别获得66.4和65.7的HOTA分数。

Conclusion: 该框架不仅适用于通用的多类跟踪，在传统基准测试上也表现出色，证明了其在通用目标跟踪中的有效性。
...
TL;DR: 提出了一种通过残差向量量化（RVQ）增强姿态码表示的方法，在保持姿态码可解释性和可操作性的同时，有效捕捉细微运动特征，显著提升了文本到运动生成的质量和可控性。


<details>
  <summary>Details</summary>
Motivation: 现有的可控运动生成方法主要依赖离散姿态码表示，但单独使用姿态码无法捕捉细粒度的运动细节，限制了运动表达的丰富性。需要一种既能保持姿态码优势又能增强细节表现的方法。

Method: 使用残差向量量化（RVQ）技术，在基于姿态码的潜在表示基础上增加连续运动特征，形成混合表示方法。这种设计结合了离散姿态码的可解释性和连续特征的细节捕捉能力。

Result: 在HumanML3D数据集上的实验显示，FID从0.041降低到0.015，Top-1 R-Precision从0.508提升到0.510。定性分析进一步证实了模型在运动编辑方面的可控性优势。

Conclusion: 通过RVQ增强的姿态码表示方法成功解决了离散姿态码在细节捕捉方面的局限性，在保持良好可控性的同时显著提升了运动生成质量，为文本到运动生成提供了更有效的解决方案。
...
Method: 采用端到端框架，第一阶段使用Transformer网络，第二阶段使用局部信息聚合网络(LIA-Net)，两阶段相互增强生成精确点运动路径。LIA-Net通过建模局部几何结构补偿Transformer网络的邻域精度损失，并利用门控循环单元引导局部位移。

Result: 在收集的数据集上与现有最先进方法相比，TCFNet取得了优异的评估指标和可视化结果。

Conclusion: TCFNet通过结合Transformer和局部几何建模，有效解决了面骨点云变换中的精度和效率问题，为正颌手术规划提供了更优的解决方案。
...
Method: 使用YOLOX-X目标检测网络从RGB图像中检测船舶，将检测结果与AIS消息融合，采用PnP和单应性变换两种方法对齐AIS数据与图像坐标，生成3D边界框表示6D位姿。

Result: PnP方法相比单应性变换显著降低了投影误差，YOLOX-X模型在相关船舶类别上达到0.80 mAP@0.5，成功创建了包含3753张图像的BONK-pose数据集。

Conclusion: 该方法能够无需人工标注自动创建6D位姿估计数据集，为船舶位姿估计网络的训练和评估提供了有价值的公开数据集资源。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有隐式神经表示方法中复杂激活函数带来的硬件开销过大问题

Method: 采用分段二次激活函数，利用其傅里叶级数中的丰富谐波内容来增强高频信号表达能力，并通过NTK分析验证，开发了统一的N级流水线框架

Result: 在FPGA和ASIC实现中，QuadINR相比现有方法PSNR提升达2.06dB，面积仅1914μm²，动态功耗6.14mW，资源消耗降低97%，功耗降低97%，延迟改善93%

Conclusion: QuadINR通过硬件友好的分段二次激活函数设计，在保持高性能的同时大幅降低了硬件开销，为隐式神经表示的硬件实现提供了高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 研究AI和机器人快速发展导致的社会变革，探索人类与机器人在共享社区中的共存机制，包括合作、竞争和社会关系建立。

Method: 开发了Virtual Community平台，包含：1、开源多代理物理模拟器，支持机器人、人类和社会交互；2、大规模真实社区生成流水线，包含室外空间、多样化室内场景和基于实际的代理社区。

Result: 提出了两个新挑战：社区规划挑战（评估多代理在开放世界中的理解和规划能力）和社区机器人挑战（要求多个异构机器人协作解决复杂任务）。评估了多种基线方法，展示了高级任务规划和低级协作控制方面的挑战。

Conclusion: Virtual Community平台为开放世界环境中人机共存的研究打开了新的可能性，希望通过该平台能够推动更多关于人类与机器人社会共存的深入研究。
...
<details>
  <summary>Details</summary>
Motivation: 随着空间转录组技术分辨率不断提高（如Visium HD达到8um），传统逐点回归方法变得低效且不稳定，且高分辨率数据的极端稀疏性和低表达水平给预测和评估带来挑战。

Method: 提出Img2ST-Net框架，将HD空间转录组数据建模为超像素表示，将图像到组学推断任务重新表述为具有数百或数千输出通道的超内容图像生成问题，采用全卷积架构并行生成密集基因表达图谱。

Result: 开发了SSIM-ST评估指标，专门针对高分辨率空间转录组分析，提高了在稀疏表达模式下的鲁棒性。框架具有可扩展性和生物学一致性。

Conclusion: Img2ST-Net为大规模高效准确的空间转录组推断提供了原则性解决方案，为下一代鲁棒且分辨率感知的空间转录组建模奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决Flux等英文训练的文本生成图像模型在处理非英语提示词（特别是中文）时表现不佳的问题，现有翻译或微调方法无法充分处理文化特定语义，影响图像真实性和质量。

Method: 采用MultiModal Diffusion Transformer (MMDiT)直接控制Flux主干，相比ControlNet类架构大幅减少参数量，同时增强对中文语义的理解能力。

Result: 实证评估显示CTA-Flux支持中英文提示词，在图像生成质量、视觉真实性和中文语义忠实度方面表现优异。

Conclusion: 该方法在不需大规模重新训练整个模型的情况下，显著提升了中文文本到图像生成的质量和文化真实性，同时保持与现有插件的兼容性。
...
<details>
  <summary>Details</summary>
Motivation: 相机拍摄屏幕时，相机色彩滤镜阵列与显示器子像素之间的频率混叠会产生严重的摩尔纹图案，现有去摩尔纹方法在处理空间变化强度、大尺度结构、通道依赖统计和时序波动等方面存在局限。

Method: 提出MoCHA-former框架，包含两个核心组件：解耦摩尔纹自适应去摩尔纹(DMAD)和时空自适应去摩尔纹(STAD)。DMAD通过摩尔纹解耦块和细节解耦块分离摩尔纹和内容，使用摩尔纹条件块生成自适应特征；STAD引入空间融合块捕获大尺度结构，特征通道注意力建模RAW帧的通道依赖性，并通过隐式帧对齐确保时序一致性。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D人体姿态估计面临深度模糊、遮挡等挑战，传统方法在利用骨架结构信息时可能忽略多尺度特征，影响估计精度。
...
Conclusion: HyperDiff通过结合扩散模型和超图卷积网络，有效提升了复杂姿态下的3D姿态估计精度，解决了深度模糊和遮挡问题。
...
Method: 提出MUSE框架，采用串联交叉注意力(CCA)机制将布局规范与文本指导无缝集成，通过显式语义空间扩展实现双向模态对齐，并使用渐进式两阶段训练策略。

Result: 实验表明MUSE能够实现零样本端到端生成，在空间准确性和身份一致性方面优于现有解决方案。

Conclusion: MUSE通过创新的CCA机制和训练策略，显著提升了可控图像合成的前沿水平，为布局可控的多主体合成提供了有效解决方案。
...
Conclusion: 多模态数据融合和交叉注意力机制显著提升了农业场景3D重建的鲁棒性和效果
...
Method: 提出DAPA框架，包含域提示机制（可学习的域特定向量）和并行交叉注意力模块（对齐前向和后向BiLSTM状态），实现域感知适应和交互同步建模。

Result: 在多个跨文化和跨语言基准测试中达到最先进性能，在NoXi-J测试集上CCC指标绝对提升0.45，并在MultiMediate'25挑战赛中获得第一名。

Conclusion: DAPA框架通过显式的域适应和交互同步建模，有效解决了对话参与度估计中的泛化性问题，为跨域人机交互系统提供了强有力的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D说话头合成方法需要大量视频数据训练每个目标身份，且音频中包含与唇部运动无关的信息，导致在少量训练帧下难以实现准确的音频到唇部映射，造成唇部同步不佳和图像质量下降。

Method: 构建静态3D高斯属性场，使用音频和面部运动信号分别控制两个独立的高斯属性变形场；设计相似性对比损失函数实现更彻底的解耦；集成粗到细模块细化渲染图像，减轻头部运动引起的模糊。

Result: 大量实验表明，D^3-Talker在有限训练数据下，在高保真渲染和准确的音频-唇部同步方面优于现有最先进方法。

Conclusion: 该方法通过解耦通用和个性化变形预测，结合对比损失和细化模块，有效解决了少量数据下的3D说话头合成问题，实现了优异的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多步扩散模型通常将正向和逆向渲染作为独立问题处理，导致循环不一致和推理速度缓慢。

Method: 提出由两个单步扩散模型组成的框架，扩展了室内外场景的本征分解，并引入循环一致性机制确保正向和逆向渲染输出的连贯性。

Result: 实验结果表明在多样化场景中达到最先进性能，相比其他基于扩散的方法推理速度显著提升，且能够以无需训练的方式迁移到视频分解。

Conclusion: Ouroboros框架通过单步扩散模型和循环一致性机制，有效解决了正向和逆向渲染的协同问题，在保持高质量的同时大幅提升效率。
...
Conclusion: DreamSwapV提供了一个有效的视频主体替换解决方案，能够实现高质量、保真度高的定制化视频编辑，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 传统统计和数据驱动方法依赖大量标注数据且在动态生产环境中适应性有限，基础模型的感知能力为工业异常检测提供了新机遇

Method: 提出PB-IAD框架，包含专门设计的提示模板用于迭代实现领域特定过程知识，以及预处理模块将用户输入转换为有效系统提示

Result: 在三种制造场景、两种数据模态下评估，结果显示在数据稀疏和低样本设置中表现优异，仅通过语义指令就超越PatchCore等先进方法

Conclusion: PB-IAD框架通过用户中心设计，使领域专家无需数据科学专业知识即可灵活定制系统，为动态生产环境中的异常检测提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 从第一人称视角预测无碰撞的未来轨迹在人形机器人、VR/AR和辅助导航等应用中至关重要，需要预测头部平移和旋转来理解主动信息收集行为。

Method: 提出了一个基于时间聚合3D潜在特征的框架，建模环境的几何和语义约束，包括静态和动态部分。使用Project Aria眼镜收集数据并构建了Aria导航数据集。

Result: 模型学会了类人的导航行为，如等待/减速、重新规划路线和环顾交通情况，并且能够泛化到未见过的环境。

Conclusion: 该方法成功预测了6D头部姿态序列，展示了在真实世界导航任务中的有效性，为学习第一人称导航策略提供了有价值的资源。
...
<details>
  <summary>Details</summary>
Motivation: 点云视频能捕捉动态3D运动并减少光照和视角变化的影响，但现有的选择性状态空间模型在处理点云视频的时空无序问题时效果有限

Method: 提出统一时空状态空间模型(UST-SSM)，包含：1)空间-时间选择扫描(STSS)通过提示引导聚类重组无序点；2)空间-时间结构聚合(STSA)补偿缺失的几何和运动细节；3)时间交互采样(TIS)增强时间依赖性
...
Conclusion: WeedSense提供了一个高效、准确的多任务杂草分析解决方案，能够满足现代农业对实时、精准杂草管理的需求，为可持续农业发展提供了有力工具。
...
TL;DR: SATURN是一个轻量级的文本到图像生成模型扩展，通过将场景图转换为显著性排序的token序列，使冻结的CLIP-VQ-VAE主干能够理解图结构，在保持先进自回归模型速度和保真度的同时显著提升布局和对象关系准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像模型在照片级渲染方面表现出色，但在处理复杂提示中的布局和对象关系时存在困难。场景图提供了自然的结构先验，但之前的图引导方法依赖于繁重的GAN或扩散管道，在速度和保真度上都落后于现代自回归架构。

Method: SATURN是VAR-CLIP的轻量级扩展，将场景图转换为显著性排序的token序列，使冻结的CLIP-VQ-VAE主干能够解释图结构，同时只微调VAR变换器。

Result: 在Visual Genome数据集上，SATURN将FID从56.45%降低到21.62%，Inception Score从16.03提高到24.78，超越了SG2IM和SGDiff等先前方法，且不需要额外模块或多阶段训练。

Conclusion: SATURN有效地将结构感知与最先进的自回归保真度相结合，在对象数量保真度和空间关系准确性方面都有显著改进。
...
Method: 采用Meta-Scenario Generation使用大语言模型推理对抗性智能体行为，然后通过Complex Scenario Evolution使用背景车辆放大核心威胁，构建对抗性协作图优化关键轨迹

Result: 在多个基于强化学习的AV模型上实验显示，ScenGE比最先进基线平均多发现31.96%的严重碰撞案例，且能应用于大模型AV系统，对抗训练可提高模型鲁棒性
...
TL;DR: WISE-FUSE是一个自适应全切片图像编码框架，通过选择性处理诊断相关区域，将WSI编码时间减少三倍以上，同时保持或超越全量处理的诊断性能
...
Method: 利用病理领域视觉语言模型和大语言模型，通过知识蒸馏机制计算低分辨率图像块与类别特定文本描述的相似度分数，选择信息丰富的区域子集进行选择性编码，并与文本嵌入融合以增强诊断上下文

Result: 实验表明WISE-FUSE将WSI编码时间减少超过三倍，同时达到与全量图像块处理相当或更好的诊断性能
...
### [47] [MF-LPR$^2$: Multi-Frame License Plate Image Restoration and Recognition using Optical Flow](https://arxiv.org/abs/2508.14797)
...
<details>
  <summary>Details</summary>
Motivation: 解决车载摄像头拍摄的车牌图像因低分辨率、运动模糊和眩光导致的识别困难问题，现有生成模型依赖预训练先验，容易产生伪影和失真

Method: 使用先进的光流估计器进行帧对齐，设计算法检测和校正错误光流估计，利用车牌图像序列的时空一致性进行多帧聚合

Result: 在PSNR、SSIM和LPIPS指标上显著优于8个最新恢复模型，识别准确率达到86.44%，远超单帧LPR(14.04%)和多帧LPR(82.55%)基线模型
...
TL;DR: 提出一种无监督OCR后处理方法，利用文档内字符形状冗余性来校正OCR输出和改进聚类效果


<details>
  <summary>Details</summary>
Motivation: 当前OCR系统在处理低质量文档时表现不佳，特别是印刷文档的域间变异性高而域内变异性低，未能充分利用文档冗余性

Method: 扩展高斯混合模型(GMM)，交替使用期望最大化(EM)算法和簇内重新对齐过程，结合正态性统计检验

Result: 在多种退化程度的文档上验证有效，包括乌拉圭军事档案和17-20世纪欧洲报纸

Conclusion: 该方法能够有效利用文档内部冗余信息提升OCR识别质量，特别适用于历史文档等低质量数据
...
### [49] [DINOv3 with Test-Time Training for Medical Image Registration](https://arxiv.org/abs/2508.14809)
*Shansong Wang,Mojtaba Safari,Mingzhe Hu,Qiang Li,Chih-Wei Chang,Richard LJ Qiu,Xiaofeng Yang*

Main category: cs.CV

TL;DR: 这篇论文提出了一种无需训练的医学图像注册方法，利用冻结的DINOv3编码器和测试时优化变形场，在两个标准数据集上都取得了优异的注册精度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的学习型医学图像注册方法需要大量训练数据，这限制了临床应用。需要一种无需训练的方法来充分利用基础模型的特征表示能力。

Method: 提出一种无训练流程，依赖冻结的DINOv3编码器，通过测试时优化特征空间中的变形场来实现图像注册。

Result: 在Abdomen MR-CT数据集上获得最高Dice分数(0.790)、最低Hausdorff距离(4.9±5.0)和最低Log-Jacobian标准差(0.08±0.02)。在ACDC心脏MRI上将Dice提高到0.769，SDLogJ降低到0.11，HD95降低到4.8。

Conclusion: 在经高级压缩的基础特征空间中进行测试时优化，为临床注册提供了一种实用且通用的无需训练的解决方案。
...
### [50] [A Comprehensive Review of Agricultural Parcel and Boundary Delineation from Remote Sensing Images: Recent Progress and Future Perspectives](https://arxiv.org/abs/2508.14558)
*Juepeng Zheng,Zi Ye,Yibin Wen,Jianxi Huang,Zhiwei Zhang,Qingmei Li,Qiong Hu,Baodong Xu,Lingyuan Zhao,Haohuan Fu*

Main category: cs.CV

TL;DR: 这是一篇关于农业田块边界划分(APBD)的综述性论文，系统总结了使用遥感图像进行农业田块检测和划分的方法发展，包括传统图像处理、传统机器学习和深度学习方法，并展望了未来研究趋势。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像为自动化农业调查提供了很好的机会，需要系统总结APBD领域的研究进展和方法发展，为研空人员提供清晰的知识地图。

Method: 进行了全面的文献综述和元数据分析，将APBD方法分为三类：传统图像处理方法、传统机器学习方法和深度学习方法，并深入讨论了深度学习方法如语义分割、目标检测和Transformer方法。

Result: 系统分析了APBD领域的算法、研究地点、作物类型、传感器类型、评估方法等元数据，识别出深度学习方法在该领域占主导地位，并讨论了多传感器数据、单任务与多任务学习等关键问题。

Conclusion: 这篇综述为APBD领域研空人员提供了全面的知识地图，展望了未来研究的潜在热门话题和应用场景，有助于跟踪该领域的发展趋势。
...
### [51] [TransLight: Image-Guided Customized Lighting Control with Generative Decoupling](https://arxiv.org/abs/2508.14814)
...
<details>
  <summary>Details</summary>
Motivation: 现有光照编辑方法无法同时提供定制化的光照效果控制和保持内容完整性，特别是在将复杂光照效果从参考图像传输到目标图像的挑战性任务中表现不佳。

Method: 提出生成式解耦策略，使用两个微调的扩散模型分离图像内容和光照效果，构建百万级图像-内容-光照三元组数据集；采用IC-Light作为生成模型，将参考光照图像作为额外条件信号进行训练。

Result: TransLight成功实现了跨不同图像的光照效果传输，提供了比现有技术更定制化的光照控制，在光照协调和编辑方面开辟了新的研究方向。

Conclusion: 该框架通过彻底解耦参考图像中的光照效果，赋予TransLight高度灵活的光照控制能力，为光照协调和编辑研究指明了新方向。
...
TL;DR: 提出LCBM框架，通过原型学习和基础模型确保概念的空间定位准确性，解决无标签概念瓶颈模型定位不准的问题


<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型需要密集人工标注，成本高昂。无标签CBMs虽然利用基础模型确定概念，但往往无法准确定位相关区域，关注视觉无关区域

Method: 为每个概念分配一个原型，通过基础模型确保原型与相关概念的关联性，利用原型学习促进准确的空间定位，学习从正确局部区域预测概念

Result: 实验结果表明LCBM能有效识别图像中的概念，改善定位性能，同时保持相当的分类性能

Conclusion: LCBM框架成功解决了无标签概念瓶颈模型的空间定位问题，通过原型学习和基础模型的结合实现了准确的概念定位
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在高光物体逆向渲染中的局限性：NeRF计算成本高，3D高斯溅射在镜面反射下表现不佳，多视角不一致导致表面噪声和结构伪影，简化渲染方程导致材质属性模糊

Method: 两阶段方法：1) 基于物理渲染和分割求和近似的鲁棒表面重建，利用基础模型的几何先验；2) 通过蒙特卡洛重要性采样进行材料分解，使用可微分2D高斯光线追踪建模间接光照，通过球面mipmap方向编码细化高频镜面细节

Result: 在几何重建、材料分离和新光照条件下的照片级真实感重照明方面达到最先进性能，优于现有逆向渲染方法

Conclusion: GOGS框架有效解决了高光物体逆向渲染中的关键挑战，实现了高质量的几何和材质重建，为复杂光照条件下的物体渲染提供了有效解决方案
...
TL;DR: TUMTraf-A数据集包含10个高速公路事故序列，提供2D/3D标注框和轨迹ID，并提出结合规则与学习的Accid3nD事故检测模型


<details>
  <summary>Details</summary>
Motivation: 尽管交通安全研究取得进展，事故仍频繁发生，需要将其视为交通网络中不可避免的偶发结果进行理解

Method: 提出Accid3nD事故检测模型，结合规则方法和学习方法，在包含路边摄像头和LiDAR数据的TUMTraf-A数据集上进行实验

Result: 实验和消融研究表明所提方法具有鲁棒性，数据集包含294,924个2D标注框和93,012个3D标注框

Conclusion: 发布了包含真实高速公路事故的TUMTraf-A数据集和Accid3nD检测模型，为事故理解提供重要资源
...
<details>
  <summary>Details</summary>
Motivation: 数字病理学中的全切片图像分析面临两个主要挑战：WSI的十亿像素级分辨率和密集监督信号的稀缺性。虽然多实例学习适合幻灯片级任务，但训练稳健模型需要大量多样化数据。现有的图像增强方法要么计算成本过高，要么缺乏对变换语义的控制。

Method: 提出HistAug方法，这是一种快速高效的生成模型，用于在数字病理学的潜在空间中进行可控增强。通过基于明确的补丁级变换（如色调、侵蚀）进行条件生成，生成真实的增强嵌入同时保留初始语义信息。该方法能够在单次前向传播中高效处理大量补丁。

Result: 在多个幻灯片级任务和不同器官上的实验表明，HistAug优于现有方法，特别是在低数据情况下。消融研究证实了学习变换相对于基于噪声的扰动的优势，并突显了统一WSI级增强的重要性。

Conclusion: HistAug提供了一种高效且有效的解决方案，通过在潜在空间中进行可控增强来改善数字病理学中的WSI分析，特别在数据稀缺情况下表现优异，为MIL模型的训练提供了重要改进。
...
TL;DR: 这篇论文提出了一种基于信息融合和不确定性估计的两阶段变换器网络，用于提高烟雾检测的稳健性和可靠性。通过光流编码和多规模不确定性学习，系统在多种评估指标上表现出艶为优称的性能。


<details>
  <summary>Details</summary>
Motivation: 烟雾呈现复杂的时空动态特征，受照明变化、流动动力学和环境噪声影响，传统检测方法的可靠性不足。需要一种可靠的早期火灾预警系统，但又不想使用复杂的多传感器数组。

Method: 提出两阶段不确定性知觉的Shifted Windows Transformer模型。首先使用四色定理启发的双阶段分数阶变分模型进行光流估计，生成颜色编码的光流图。然后通过高斯混合模型融合外观线索，生成烟雾区域的二值分割掩码。最后使用增强了多尺度不确定性估计头的Shifted-Windows Transformer进衄两阶段学习：第一阶段优化检测准确性，第二阶段同时学习认别性和认知不确定性来估计预测的可靠性。

Result: 通过多种评估指标和与最新方法的对比分析，证明了该方法具有超越的普适性和稳健性。

Conclusion: 该框架为监控、工业安全和自主监测应用提供了一种可靠的早期火灾检测解决方案，通过信息融合和不确定性学习有效提升了烟雾检测的可靠性。
...
TL;DR: 本文分析了视觉提示方法在增量目标检测(IOD)中的应用，发现在复杂域增量学习设置中，单纯的提示方法表现不佳，但结合少量数据回放的提示方法效果最佳。


<details>
  <summary>Details</summary>
Motivation: 视觉提示方法在增量学习图像分类中表现出色，但尚未应用于增量目标检测，需要验证其通用性和有效性。

Method: 在复杂域增量学习设置下分析三种不同的提示方法，并提供广泛基线进行比较，同时测试提示长度和初始化策略。

Result: 测试的提示方法在该设置下表现不佳，但结合少量数据回放的视觉提示方法取得了最佳结果。

Conclusion: 研究为推进提示方法在增量目标检测中的应用提供了有价值的见解，表明结合回放策略的提示方法具有实用性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络在视觉任务中主要局限于图像分类等简单任务，而在标准RGB视频流中处理复杂时序任务（如多目标跟踪）的潜力尚未充分探索。

Method: 提出SMTrack框架，引入自适应尺度感知归一化Wasserstein距离损失(Asa-NWDLoss)来提升不同尺度和密度下的检测定位性能，并采用TrackTrack身份模块保持目标轨迹的一致性。

Result: 在BEE24、MOT17、MOT20和DanceTrack数据集上的广泛评估表明，SMTrack达到了与领先ANN方法相当的性能水平。

Conclusion: 该工作推进了SNN在复杂场景下鲁棒准确的多目标跟踪应用，为脉冲神经网络在实时视觉任务中的应用开辟了新方向。
...
TL;DR: AnchorSync是一个基于扩散模型的视频编辑框架，通过分离锚点帧编辑和中间帧插值来解决长视频编辑中的全局一致性和时序连贯性问题


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理分钟级长视频时容易出现结构漂移和时序伪影，难以保持全局一致性和时序连贯性

Method: 将长视频编辑任务解耦为稀疏锚点帧编辑和平滑中间帧插值，通过渐进去噪过程保持结构一致性，利用多模态指导保留时序动态

Result: 实验表明AnchorSync能够产生连贯、高保真的编辑效果，在视觉质量和时序稳定性方面超越现有方法
...
TL;DR: PerSense是一个无需训练、模型无关的单样本密集图像个性化实例分割框架，通过密度图生成实例级候选点提示，并采用自适应阈值和空间门控过滤误报。PerSense++增强版本增加了多样性感知范例选择、混合提示生成和无关掩码拒绝模块，在密集场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 密集视觉场景中的分割面临遮挡、背景杂乱和尺度变化的挑战，现有方法在密集环境下效果有限，需要开发专门处理密集图像的个性化分割框架。

Method: 使用Instance Detection Module通过密度图生成实例级候选点提示，Point Prompt Selection Module通过自适应阈值和空间门控过滤误报，反馈机制自动选择有效范例改进密度图质量。PerSense++增加多样性感知范例选择、混合IDM（结合轮廓和峰值提示生成）和Irrelevant Mask Rejection Module。

Result: 在多个基准测试上的广泛实验表明，PerSense++在密集设置下优于现有方法。

Conclusion: 提出的PerSense框架及其增强版本PerSense++能够有效处理密集图像中的个性化实例分割任务，并通过新基准PerSense-D支持这一未充分探索的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要清晰图像进行相机姿态估计和点云生成，这在严重运动模糊情况下不现实，COLMAP初始化方法也会因特征对应不可靠而失败。
...
Conclusion: GeMS框架成功解决了从极端运动模糊图像直接进行3D重建的挑战，无需依赖清晰参考图像，为模糊场景重建提供了有效解决方案。
...
TL;DR: 这篇论文提出了一种模型驱动的视觉基础模型训练方法，通过联合知识转移和保持技术，统一多个预训练模型来构建功能强大的通用视觉模型，避免了传统数据驱动方法对大规模标签数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 解决以数据为中心的视觉基础模型开发方法对大规模高质量标签数据和高端GPU资源的依赖，让普通机构也能发展功能强大的通用视觉模型。

Method: 通过在共享潜在空间中统一多个预训练教师模型，缓解分布差异导致的"不平衡转移"问题，并使用适配器模块以通用教师为知识库整合专门领域教师的知识。

Result: 实验结果显示，该模型在四个核心视觉任务（图像分类、目标检测、语义分割和实例分割）上都超过了现有的数据为中心模型。

Conclusion: 通过统一和聚合现有模型，可以构建功能强大的通用视觉基础模型，继承各专业领域模型的知识，而无需大量标签数据训练，为视觉AI开发提供了新的方向。
...
TL;DR: GSFix3D是一个新颖框架，通过将扩散模型的先验知识蒸馏到3D表示中，提升3D高斯泼溅在极端视角和部分观测区域的渲染质量，同时保持与观测场景细节的一致性。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在新视角合成方面取得了显著进展，但在极端新视角或部分观测区域生成高质量渲染仍然具有挑战性。同时，扩散模型虽然具有强大的生成能力，但依赖文本提示且缺乏对特定场景信息的感知，阻碍了准确的3D重建任务。

Method: 提出了GSFix3D框架，核心是GSFixer（一个通过定制微调协议获得的潜在扩散模型），能够利用网格和3D高斯来适应预训练生成模型到不同环境和重建方法的伪影类型。还提出了随机掩码增强策略，使GSFixer能够合理修复缺失区域。

Result: 在具有挑战性的基准测试中，GSFix3D和GSFixer实现了最先进的性能，仅需在捕获数据上进行最少的场景特定微调。真实世界测试进一步证实了其对潜在姿态错误的鲁棒性。

Conclusion: GSFix3D通过结合扩散模型的生成能力和3D高斯泼溅的表示能力，有效解决了极端视角和部分观测区域的渲染质量问题，为3D重建和新视角合成提供了强有力的解决方案。
...
TL;DR: 提出了一种基于多尺度视频Transformer的高效类无关分割方法，仅使用运动线索检测未知物体，无需光流计算，在保持高分辨率信息的同时实现了实时性能
...
Method: 使用多阶段多尺度查询-记忆解码机制，结合尺度特定的随机丢弃令牌技术，采用共享可学习记忆模块来保持详细的时空特征
...
Method: 使用定制化的可调光谱光照箱，在390种光照条件下使用4台不同相机捕获18个场景，构建首个此类数据集。基于此数据集开发轻量级神经网络方法进行光照和传感器映射。
...
TL;DR: 提出了一种结合事件相机和RGB相机的6-DoF姿态跟踪方法，通过事件光流测量高速运动物体的速度，结合低频全局姿态估计实现高速运动下的精确姿态跟踪。


<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在高速度运动场景下存在帧率限制和运动模糊问题，而事件相机具有高时间分辨率、低延迟和高动态范围的优势，但缺乏丰富的视觉信息。需要结合两种传感器的优势来解决高速运动物体的6-DoF姿态跟踪问题。

Method: 使用事件光流算法测量物体运动，实现6-DoF速度跟踪器，然后将跟踪到的物体速度与低频全局姿态估计器（基于RGB相机）的估计姿态进行积分融合。

Result: 在合成数据和真实数据上测试验证，证明了该方法的有效性，特别是在高速运动场景下表现优异。

Conclusion: 通过结合事件相机和RGB相机的优势，成功实现了高速运动物体的6-DoF姿态跟踪，解决了传统方法在高速场景下的局限性。
...
TL;DR: 本研究首次系统性分析了病理基础模型中的医院域偏差问题，提出了轻量级对抗框架来消除医院特异性特征，在保持疾病分类性能的同时显著降低域可预测性。
...
Conclusion: 提出的对抗框架能有效缓解病理基础模型中的医院偏差问题，为PFM的临床部署提供了重要解决方案，特别是在跨医院泛化场景中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有技术需要大量逐场景优化来保证多视角一致性，Tinker旨在从少量图像实现鲁棒的多视角一致性编辑，降低通用3D内容创建的门槛。

Method: 开发了参考多视角编辑器和任意视角到视频合成器两个新组件，利用预训练扩散模型的潜在3D感知能力和视频扩散的空间-时间先验。

Result: 在编辑、新视角合成和渲染增强任务上达到最先进性能，显著减少了通用3D内容创建的障碍。

Conclusion: Tinker代表了向真正可扩展、零样本3D编辑的关键一步，为这一领域的研究提供了首个大规模多视角编辑数据集和数据管道。
...
TL;DR: EventSSEG是一种基于事件相机的事件计算道路分割方法，采用概率注意力机制和自监督学习，在DSEC-Semantic和DDD17数据集上实现了最先进性能，且仅需少量标注数据。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆需要低延迟、低计算量的道路分割方案。事件相机具有低功耗优势，但缺乏标注数据且难以从传统相机预训练权重迁移，需要新的解决方案。

Method: 提出EventSSEG方法，使用纯事件计算和概率注意力机制，采用事件自监督学习避免对大量标注数据的依赖。

Result: 在DSEC-Semantic和DDD17数据集上实现了最先进的性能，仅需极少量标注事件数据。

Conclusion: 该方法充分发挥了事件相机的优势，有效解决了事件数据标注稀缺的问题，为自动驾驶道路分割提供了高效解决方案。
...
TL;DR: 通过AI基于CT和MRI图像分析研究腺脾形态随麄龙变化趋势，发现2型糖尿病患者的腺脾大小和形状显著异常


<details>
  <summary>Details</summary>
Motivation: 理解腺脾变化对于检测2型糖尿病和其他腺脾疾病至关重要，需要建立腺脾形态随麄龙变化的标准规范

Method: 分析2533名患者的腺部CT或MRI图像，使用自动化方法分割腺脾并提取13个形态特征，通过GAMLSS回归模型分析麄龙趋势

Result: 在调整混杂因素后，13个形态特征中有10个在2型糖尿病患者与对照组之间存在显著差异，腺脾在2型糖尿病患者中更小

Conclusion: 研究提供了腺脾形态随麄龙变化的参考标准，证实2型糖尿病导致腺脾小化，为临床诊断提供了重要依据
...
<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法依赖单一骨架规范，限制了在不同关节结构和解剖覆盖的数据集上的泛化能力

Method: 提出MS-CLR自监督框架，从同一序列中提取多个骨架规范并对齐姿态表示，使用改进的ST-GCN架构处理不同关节布局和尺度的骨架

Result: 在NTU RGB+D 60和120数据集上，MS-CLR持续优于单骨架对比学习方法，多骨架集成进一步提升了性能，达到了新的最先进结果

Conclusion: 多骨架对比学习能够学习结构不变性和捕捉多样解剖线索，产生更具表达力和泛化能力的特征
...
TL;DR: 提出了一种使用铰接3D高斯模型统一表示几何和运动的铰接物体重建方法，显著提升了多部件复杂物体的重建能力，并创建了包含90个物体的新基准MPArt-90。


<details>
  <summary>Details</summary>
Motivation: 现有方法将几何和运动解耦处理，先重建不同状态下的物体形状，再通过后处理对齐估计铰接关系，这种分离使重建流程复杂且限制了可扩展性，特别是对于具有复杂多部件铰接的物体。

Method: 引入统一的铰接3D高斯表示，联合建模几何和运动，提高了运动分解的鲁棒性，支持多达20个部件的铰接物体。

Result: 在广泛的物体类型上，该方法在部件级几何重建和运动估计方面始终实现卓越精度，显著优于先前方法（先前方法通常在2-3个部件以上就难以处理）。

Conclusion: 该方法展示了统一铰接表示在机器人仿真和人机交互建模等下游任务中的应用潜力，突显了在可扩展物理建模中的重要价值。
...
TL;DR: 将视觉叙事视为图像描述的超集，先通过视觉-语言模型生成图像描述，再用语言-语言方法转换为连贯故事，提升故事质量并加速训练


<details>
  <summary>Details</summary>
Motivation: 平衡视觉叙事的两个关键方面：既要基于图像序列又要保持叙事连贯性，通过统一框架整合图像描述和故事生成

Method: 采用两阶段方法：1) 使用视觉-语言模型为输入图像生成描述 2) 使用语言-语言方法将描述转换为连贯叙事

Result: 统一框架对生成故事质量有积极影响，相比先前研究加速了训练时间，框架易于复用和重现

Conclusion: 提出的方法有效平衡了视觉叙事的不同需求，并引入了新的ideality指标来模拟与理想模型的接近程度，可用于评估人类相似性
...
TL;DR: 首个斯瓦希里语NLP的分类学指导评估，通过健康心理测量任务收集2170个肯尼亚语者自由文本，分析部落影响、城市方言、语码转换和借词现象，建立分类学框架评估模型预测错误。


<details>
  <summary>Details</summary>
Motivation: 解决斯瓦希里语NLP研究中社会语言多样性不足的问题，填补该领域在文化背景和语言变异方面的评估空白。

Method: 收集肯尼亚语者的自由文本响应数据，开发结构化分类学，使用预训练和指令调优语言模型进行预测错误分析。

Result: 数据显示出明显的部落文化影响、城市方言特征、语码混合和借词现象，模型性能受到社会语言变异的显著影响。

Conclusion: 研究推进了文化基础评估框架的发展，强调了社会语言变异在模型性能塑造中的重要作用，为多语言NLP评估提供了新视角。
...
TL;DR: 本文通过LLM注释的英汉新闻语料库，从功能块角度分析英汉新闻在状语成分顺序上的差异，发现英语偏向核心信息先行、功能块后置，汉语则偏向背景先行、功能块前置的特征。


<details>
  <summary>Details</summary>
Motivation: 探索英汉新闻在功能块成分顺序上的差异，分析其位置偏好和分布模式，为对比语言学提供新的实证支持。

Method: 基于大语言模型注释的可比英汉新闻语料库，从功能块角度对比分析英汉新闻中具有状语功能的成分顺序差异。

Result: 1）英语新闻偏向核心信息先行的线性叙述，功能块多后置；汉语新闻偏向背景先行的整体呈现，功能块常前置。2）SVO结构中两者均有差异，汉语前置倾向更显著。3）功能块共现时，两者都显示高灵活性，顺序调整受信息和语用目的驱动。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的表格事实核查解决方案对非专家用户不够友好，需要开发一个易于使用的交互式工具来让非专家也能访问先进的事实核查技术。

Method: 开发了T-REX系统，使用最先进的指令调优推理大语言模型，支持多模态和多语言表格数据的声明验证，强调准确性和透明度。

Result: 成功创建了首个实时交互式表格事实核查工具，该系统已在线公开可用，为非专家用户提供了先进的事实核查技术访问途径。

Conclusion: T-REX通过交互式界面和先进的语言模型技术，成功降低了表格事实核查的技术门槛，使非专家用户也能进行准确的声明验证。
...
TL;DR: 本文研究了基于大语言模型的文本到SQL任务的置信度估计方法，比较了黑盒和白盒策略，发现一致性方法在黑盒模型中表现最佳，而SQL语法感知方法在白盒设置中更有优势，执行结果能进一步提升两种方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 文本到SQL任务需要评估模型生成SQL查询的可靠性，但在大语言模型场景下，模型权重和梯度的访问受限，因此需要研究有效的置信度估计方法。

Method: 探索了黑盒和白盒置信度估计策略，包括一致性方法和SQL语法感知方法，并在跨域文本到SQL基准上评估其有效性，同时利用执行结果作为补充信号。

Result: 一致性方法在黑盒模型中表现最优，SQL语法感知方法在白盒设置中具有优势，执行结果能有效提升两种方法的性能。

Conclusion: 研究为文本到SQL任务的置信度估计提供了有效方法，特别是在大语言模型受限访问的场景下，结合执行结果能进一步提升估计准确性。
...
TL;DR: 论文分析了微调LLMs时的数据记忆隐私风险，提出了多层隐私保护框架，在保持94.7%模型效用的同时将数据泄露降至0%。
...
Result: 实验显示重复敏感数据微调使隐私泄露率从0-5%升至60-75%，提出的保护方法可将泄露降至0%同时保持94.7%的模型效用。

Conclusion: 多层隐私保护框架能有效解决LLMs微调中的数据记忆问题，在保护隐私的同时维持模型性能。
...
TL;DR: 该研究探讨了大语言模型中信息收集和传播的机制，重点关注标点符号的计算重要性以及不同推理规则的处理方式，发现了模型间的显著差异。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型中信息如何在不同层间收集和传播，特别是标点符号作为注意力汇聚点和记忆辅助工具的计算重要性，以及不同推理规则的处理机制。

Method: 使用基于干预的技术，在GPT-2、DeepSeek和Gemma模型中进行标点符号的必要性和充分性评估，通过交换干预和层交换实验分析条件语句和全称量化等推理规则的处理方式。

Result: 发现模型间存在显著差异：GPT-2中多个层的标点符号既必要又充分，DeepSeek中这种情况较少，而Gemma中完全不成立。条件语句和全称量化等推理规则的处理方式也存在明显不同。

Conclusion: 研究揭示了LLM中标点符号使用和推理的内部机制，为模型可解释性提供了新的见解，表明不同模型在处理相同组件时采用不同的策略。
...
<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型(DLLMs)在非自回归文本生成方面表现出潜力，但直接应用现有的后训练量化方法会导致严重的精度下降和泛化性能降低，需要专门针对DLLMs特性的量化解决方案。

Method: 提出了DLLMQuant框架，包含三项核心技术：1)时间-掩码自适应采样(TMAS)考虑时间和掩码因素的校准方法；2)交互感知激活量化(IA-AQ)利用双向注意力交互信号动态分配量化资源；3)确定性引导量化(CGQ)整合掩码状态和token分数作为误差补偿的关键权重标准。

Result: 实验表明DLLMQuant在提升效率的同时实现了显著的性能增益，相比现有方法(如AWQ在W4A4配置下精度下降16%)有大幅改进。

Conclusion: DLLMQuant成功解决了DLLMs量化中的三个核心问题：迭代生成导致的分布变化、量化误差累积放大以及掩码/非掩码token分布不兼容问题，为DLLMs的高效部署提供了有效的量化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着学术出版物快速增长，同行评审变得至关重要但耗时。现有LLM辅助评审缺乏统一的评估基准，特别是在处理包含图表等多模态内容的场景中
...
TL;DR: DPad是一种无需训练的方法，通过滑动窗口和距离衰减dropout策略，显著降低扩散大语言模型的计算开销，实现61.4倍加速同时保持准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型(dLLMs)在并行文本生成时存在高计算开销问题，因为它们在每一步都预测所有未来后缀token但只保留一小部分，造成大量冗余计算。

Method: 提出Diffusion Scratchpad (DPad)方法，包含两种策略：(1)滑动窗口保持固定长度的后缀窗口；(2)距离衰减dropout在注意力计算前确定性地移除远处后缀token。该方法与现有优化技术兼容且代码实现简单。

Result: 在LLaDA-1.5和Dream模型上的多基准测试表明，DPad相比原始dLLMs实现了高达61.4倍的加速，同时保持了相当的准确率。

Conclusion: DPad展示了在高效和可扩展的长序列推理方面的潜力，为扩散语言模型的实用化提供了有效解决方案。
...
TL;DR: 本研究系统评估了文本分类推理中模型准确性与能耗的权衡，发现最佳准确率的模型也能实现能效，而大型LLM能耗高但准确率低，执行时间可作为能耗的有效代理指标。
...
Result: 发现最佳准确率模型同样能效优异，大型LLM能耗显著更高但分类准确率更低；推理能耗存在巨大差异（<mWh到>kWh），与模型运行时间强相关。
...
TL;DR: 论文提出了SciIG任务来评估LLM从标题、摘要和相关工作中生成研究论文引言的能力，通过多维度评估发现LLaMA-4 Maverick表现最佳，三样本提示效果最好。
...
Method: 从NAACL 2025和ICLR 2025论文中整理新数据集，评估5个最先进模型（包括开源和闭源系统），使用结合自动指标和LLM作为评判的全面评估框架。

Result: LLaMA-4 Maverick在大多数指标上表现最佳，特别是在语义相似性和忠实度方面。三样本提示始终优于少样本方法。

Conclusion: 这些发现为开发有效的研究写作助手提供了实用见解，并为LLM辅助学术写作设定了现实期望。所有代码和数据集将公开释放以促进可重复性和未来研究。
...
TL;DR: 该论文提出通过多语言概念激活平均的方法，从LLMs中提取更纯净的概念语义表示，相比单语言能更好地对齐本体类之间的真实关系。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs中语义与句法和语言特定信息纠缠的问题，通过多语言平均来分离出更纯粹的概念语义表示。

Method: 使用稀疏自编码器获取Gemma 2B LLM中本体类在不同语言（英语、法语、中文）下的概念激活，然后对这些多语言激活进行平均得到概念平均值。

Result: 概念平均值相比单语言激活能更好地与本体类之间的真实映射关系对齐，表明该方法能更准确地解释网络内部状态。

Conclusion: 多语言概念激活平均是一种新的技术，能够以更高精度实现LLMs内部状态的机制性解释，为连接形式知识表示与LLMs提供了有前景的方法。
...
TL;DR: GRILE是首个罗马尼亚语教育基准测试，包含1151个选择题，用于评估多语言和罗马尼亚语LLMs在答题准确性和解释质量方面的表现。研究发现大多数开源模型准确率低于65%，48%的解释存在事实或教学错误。
...
Method: 创建GRILE基准测试，包含来自罗马尼亚高风险考试的1151个选择题，评估7个最先进的多语言和罗马尼亚语专用LLMs的答题准确性和解释质量，并进行专家评审。

Result: Gemini 2.5 Pro达到83%准确率，但大多数开源模型低于65%，48%的解释存在事实或教学缺陷。系统性问题集中在形态学和最新DOOM3正字法规范应用方面。

Conclusion: GRILE揭示了低资源教育NLP的可信度挑战，为可控解释生成和评估提供了新的测试平台，所有数据和代码已开源以促进未来研究。
...
TL;DR: 提出了一种结合基于规则的形态分析和统计子词分割的混合分词框架，特别针对形态丰富的语言如土耳其语，在TR-MMLU基准测试中取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有的子词分词方法（如BPE和WordPiece）主要依赖频率统计，在处理形态丰富和粘着语时效果不佳，因为它们忽略了语言结构信息。

Method: 使用音位归一化、词根-词缀词典和新型算法来平衡语素保持与词汇效率，为音位变体词缀和变化词根形式分配共享标识符，并集成BPE处理未登录词。

Result: 在TR-MMLU基准测试中，该分词器获得了最高的土耳其语分词百分比（90.29%）和纯分词百分比（85.8%），相比LLaMA、Gemma和GPT的分词器产生了更语言学意义和连贯的分词结果。
...
TL;DR: 提出了一个联合多任务模型用于UD形态句法分析共享任务，使用共享编码器和三个专用解码器，在九种语言上取得了最佳性能


<details>
  <summary>Details</summary>
Motivation: 解决Universal Dependencies 2025形态句法分析共享任务中同时预测形态和句法分析的需求
...
Conclusion: 模型性能依赖于正确的分词和内容词识别，在核心语法格（特别是主格-宾格）和名词特征方面仍存在挑战
...
TL;DR: Finch-Zk是一个黑盒框架，通过跨模型一致性检测来识别和缓解LLM幻觉，无需外部知识源，在检测和缓解方面都取得了显著效果提升。
...
Method: 采用跨模型一致性检查策略，通过比较不同模型对语义等价提示的响应来发现细粒度不准确性；使用针对性缓解技术对问题片段进行精确修正。

Result: 在FELM数据集上，幻觉检测F1分数比现有方法提升6-39%；在GPQA-diamond数据集上，应用于Llama 4 Maverick和Claude 4 Sonnet等先进模型时，答案准确率绝对提升7-8个百分点。

Conclusion: Finch-Zk为生产LLM系统提供了实用、可部署的安全保障，能有效增强事实可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的综述生成方法在长篇幅多章节综述中难以保持连贯性，且引用覆盖不全面，需要新的解决方案。

Method: 结合粗到细检索、自适应规划和记忆引导生成。先进行综述级检索构建初始大纲和写作计划，在生成过程中通过记忆机制动态优化，当上下文不足时触发细粒度子章节检索。

Result: 在四个科学领域的实验中，SurveyGen-I在内容质量、一致性和引用覆盖方面均优于先前工作。

Conclusion: SurveyGen-I框架通过创新的检索和记忆机制，有效解决了自动综述生成中的连贯性和引用完整性挑战，为科学文献综述自动化提供了可靠解决方案。
...
TL;DR: 本文提出了一种行为对齐检索器(BAR)，通过检索行为一致的演示样本来帮助大语言模型做出更准确的外部工具调用决策，减少错误调用并保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过微调或基于演示的提示来增强大语言模型的工具调用能力，但存在训练开销大和演示样本不一致的问题，导致模型调用行为被误导。

Method: 构建包含调用和不调用两种行为的数据集，使用对比学习框架训练BAR检索器，采用定制化的正负样本对和双重负对比损失函数来确保检索行为一致的示例。

Result: 实验表明该方法显著减少了错误函数调用，同时保持了高任务性能，为工具增强的大语言模型提供了成本效益高的解决方案。

Conclusion: 行为对齐检索器(BAR)是一种有效的方法，能够通过提供行为一致的演示样本来改善大语言模型的工具调用准确性，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个可控、标准化的访谈系统，用于跟踪态度形成和行为变化等场景，使定性数据收集更加系统和可量化。

Method: 采用低计算量的非生成式方法构建对话系统，通过在线管理面板实现无代码调整，支持创建新的访谈流程。

Result: 成功开发出可定制的访谈系统，并在COVID-19情感访谈和神经技术公众意见调查两个案例中得到应用验证。

Conclusion: 该系统为标准化访谈数据收集提供了有效工具，代码开源便于扩展功能，具有较好的可访问性和实用性。
...
TL;DR: 该研究提出了ZPD-SCA基准，评估大语言模型在判断中文阅读材料认知难度与学生发展阶段匹配度的能力，发现LLMs在零样本学习中表现不佳，但在上下文学习中有显著提升。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在教育应用中展现出潜力，但其在评估阅读材料与学生认知发展阶段（基于最近发展区理论）匹配度方面的能力尚未得到充分探索，特别是在中文教育背景下缺乏全面研究。

Method: 引入ZPD-SCA基准，由60位特级教师（全国前0.15%的在职教师）进行标注，专门用于评估阶段性的中文阅读理解难度，并测试LLMs在零样本和上下文学习场景下的表现。

Result: LLMs在零样本学习中表现较差（Qwen-max和GLM甚至低于随机猜测概率），但在提供上下文示例后性能大幅提升，部分模型准确率接近零样本基线的两倍。模型表现出系统性方向偏差，且在不同文体间性能差异显著。
...
TL;DR: 提出基于提示的LLM置信度校准框架，通过游戏化反馈机制动态改善模型校准效果，无需额外监督或参数更新


<details>
  <summary>Details</summary>
Motivation: 随着LLM在关键决策领域部署增多，需要确保其置信度估计与实际正确性相符，现有方法大多需要额外监督或参数调整

Method: 受Credence Calibration Game启发，建立结构化交互循环，通过反馈驱动提示和自然语言性能总结来动态改进模型校准

Result: 跨模型和游戏配置的广泛实验显示评估指标持续改善

Conclusion: 基于游戏的提示策略是LLM校准的有效方法，代码和数据已开源
...
TL;DR: DEPTH框架通过依赖感知句子简化和双层层次化精炼，显著降低关系抽取中的幻觉问题，在6个基准测试中平均幻觉率降至7.0%，F1分数提升17.2%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在关系抽取中经常产生虚假预测，特别是在复杂句子结构或语义情况下，这会导致知识图谱中出现噪声边，影响结构化知识的完整性和下游应用的可靠性。
...
Result: 在六个基准测试中，DEPTH将平均幻觉率降低到7.0%，同时平均F1分数相比最先进基线提高了17.2%。

Conclusion: DEPTH通过依赖感知简化和层次化精炼有效解决了关系抽取中的幻觉问题，显著提升了关系抽取的准确性和可靠性。
...
TL;DR: 本文研究发现LLMs在单独文本识别范式(IPP)下难以区分自己生成的文本，提出了"隐式领域意识"概念和"认知手术"框架来唤醒这种能力，显著提升了识别准确率。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明LLMs在成对文本范式(PPP)下能可靠识别自己生成的文本，但在单独文本范式(IPP)下性能急剧下降，这种现象的原因尚未得到系统分析。

Method: 提出Cognitive Surgery (CoSur)框架，包含四个模块：表示提取、领域构建、作者判别和认知编辑，通过唤醒LLMs的隐式领域意识(ITA)来提升IPP下的识别能力。

Result: 实验结果表明，该方法显著提升了三种不同LLM在IPP场景下的性能，平均准确率分别达到83.25%、66.19%和88.01%。

Conclusion: LLMs具有隐式的自我文本识别能力，但需要特定方法唤醒；CoSur框架有效解决了IPP范式下的识别问题，为理解LLMs的自我认知机制提供了新视角。
...
TL;DR: 该论文提出了一种基于知识图谱注入的微调算法框架，通过图神经网络编码实体关系，设计融合机制联合建模知识图谱嵌入和语言模型上下文表示，使用门控机制动态平衡语言语义和结构知识的贡献，有效提升大语言模型在结构化知识任务中的表现。
...
Method: 基于预训练语言模型，引入结构化图信息进行辅助学习，使用图神经网络编码实体及其关系，构建基于图的语义表示，设计融合机制联合建模知识图谱嵌入和语言模型上下文表示，引入门控机制动态平衡不同表示空间的贡献，构建联合损失函数同时考虑任务性能和结构对齐目标。

Result: 实验结果表明，所提出的结构感知微调框架显著增强了模型表示复杂语义单元的能力，在实体识别、问答和语言生成等任务中表现出更好的语义一致性和上下文逻辑建模能力。
...
Method: 采用Nemotron-H架构，将大部分自注意力层替换为Mamba-2层；先预训练120亿参数基础模型，然后通过Minitron策略进行压缩和蒸馏，支持在单张A10G GPU上推理128k tokens。

Result: 在推理基准测试中达到与Qwen3-8B等同类模型相当或更好的精度，同时在8k输入16k输出的推理场景中实现高达6倍的推理吞吐量提升。

Conclusion: 该模型成功展示了混合Mamba-Transformer架构在提升推理效率方面的优势，为长序列推理任务提供了高效的解决方案，相关模型和数据集已在Hugging Face发布。
...
TL;DR: In2x团队为WMT25机器翻译共享任务开发的开放系统，专注于日语相关翻译，探索将大语言模型扩展到其他语言的通用范式，包括数据构建方法和奖励模型设计。


<details>
  <summary>Details</summary>
Motivation: 旨在解决低资源和小语种语言在大语言模型中的性能问题，探索通用的方法来扩展LLMs到其他语言，特别是日语相关翻译任务。

Method: 采用包含数据构建方法和奖励模型设计的通用范式，通过系统化的方法来扩展大语言模型的多语言能力。

Result: 本文描述了WMT25共享任务的开放系统提交方案，但具体性能结果需要在任务评估后确定。

Conclusion: 提出的通用范式有望帮助大语言模型系统在低资源和小语种语言中实现卓越性能，为多语言机器翻译提供可扩展的解决方案。
...
TL;DR: 本文提出了一种中间表示方法（RLS）来捕捉自然语言论证的逻辑结构，包括逻辑原子和推理规则，从而支持多种推理任务和解释生成。


<details>
  <summary>Details</summary>
Motivation: 当前基于规则的推理方法在可解释性和扩展性方面存在局限，无法支持溯因推理和矛盾识别等任务，需要一种能够理解论证逻辑结构的表示方法。
...
Conclusion: RLS表示方法有效解决了当前推理系统的局限性，支持多种推理形式、实时错误修正和交互式讨论，为逻辑推理提供了更强大的基础。
...
TL;DR: 通过四元数姿态编码和对比损失增强Transformer手语生成模型，提高对签名者形态和风格变化的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 解决手语生成中因签名者形态和风格多样性导致的高类内变异问题，提高模型对变化的鲁棒性

Method: 1) 使用四元数空间中的骨骼旋转编码姿态，采用测地线损失提高关节角度运动的准确性；2) 引入对比损失，基于gloss重叠或SBERT句子相似性来结构化解码器嵌入，过滤不相关的解剖和风格特征

Result: 在Phoenix14T数据集上，仅使用对比损失就比PT基线提高了16%的关键点正确概率；结合四元数姿态编码后，平均骨骼角度误差降低了6%
...
TL;DR: 为南乌兹别克语建立了新的机器翻译资源，包括平行语料库、微调模型和防止半空格处理方法


<details>
  <summary>Details</summary>
Motivation: 南乌兹别克语虽有500万使用者，但在自然语言处理领域中表现不足，需要更多资源支持

Method: 收集平行语料库（997句FLORES+ dev集和39,994句平行句），微调NLLB-200模型，并提出防止阿拉伯文字半空格字符的后处理方法

Result: 开发了专门的lutfiy模型，提高了语法边界处理能力，所有资源公开发布

Conclusion: 该研究为南乌兹别克语和其他低资源语言提供了重要的机器翻译资源支持
...
<details>
  <summary>Details</summary>
Motivation: 传统词典工具在文学文本情感分析中表现不佳，特别是对低资源语言；而transformer模型通常输出粗粒度的分类标签，限制了细粒度分析。文学文本因比喻性语言、风格模糊性和情感唤起策略而具有独特挑战。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然表现出强大的类比学习能力（上下文学习），但近期研究发现这种能力存在局限性，特别是在处理一阶量词（如all、some）和线性函数的任务上。Softmax作为注意力机制中的评分函数被认为是造成这些限制的因素之一。

Method: 提出了scaled signed averaging (SSA)作为Softmax的替代方案。在编码器-解码器和仅解码器的Transformer模型上进行了评估，比较了SSA与基于Softmax的模型在各种语言探测任务上的表现。

Result: 实证结果显示SSA在目标任务上显著提升了性能。使用SSA的Transformer模型在各种语言探测任务上匹配或超越了基于Softmax的对应模型。

Conclusion: SSA作为一种新的评分函数，有效解决了Softmax在特定任务上的局限性，为改进大型语言模型的上下文学习能力提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统大语言模型在中医领域应用受限，主要因为两个关键障碍：(1)高质量中医数据稀缺；(2)中医诊断本质上是多模态的（望闻问切），涉及视觉、听觉、嗅觉和脉诊等多种感官模态，超出了传统LLMs的能力范围。

Method: 构建了迄今为止最大的中医数据集（100GB文本+200GB多模态数据，包含120万张图像、200小时音频和生理信号），对ShizhenGPT进行预训练和指令微调，使其具备深厚的中医知识和多模态推理能力。建立了中医资格考试和视觉诊断基准进行评估。

Result: 实验表明ShizhenGPT在可比规模LLMs中表现最优，甚至能与更大的专有模型竞争。在现有多模态LLMs中，ShizhenGPT在中医视觉理解方面领先，并展示了在声音、脉搏、气味和视觉等多种模态上的统一感知能力。
...
TL;DR: 本文建立了严格的文本食谱生成基准，通过针对性分词策略增强词汇表，大幅提升了大型Transformer模型在食谱生成任务上的性能表现
...
Method: 使用针对性的分词策略，在词汇表中添加23个常见分数标记和自定义结构标记，对比微调的GPT-2大型模型(774M)与GPT-2小型模型(124M)以及传统LSTM/RNN基线模型

Result: 大型Transformer方法相比最佳循环基线在BERTScore(F1)上获得>20%的相对提升(0.92 vs 0.72)，困惑度降低69.8%

Conclusion: 该基础研究为整合现实世界约束和多模态输入的高级食谱生成研究铺平了道路，但事实准确性方面仍存在挑战
...
<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法主要关注词汇级重写，产生语义相同的变体。虽然LLMs通过知识涌现能力增强了文本增强，但控制输出风格和结构仍然具有挑战性，需要精细的提示工程。
...
Conclusion: LMTransplant通过充分利用LLMs中嵌入的知识，能够生成更多样化和创造性的内容级变体，同时保留原始文本的核心属性，是一种有效的文本增强方法。
...
TL;DR: 这篇论文通过逻辑基础的合成语料评估大型语言模型在多语言自然语言推理中的一致性，发现语码切换不仅不降低性能而且可能提高性能，指出这可能是一种正则化信号。


<details>
  <summary>Details</summary>
Motivation: 语言模型在多语言环境中的应用日益增多，但它们在不同语言间的一致性和逻辑基础对齐能力仍然未得到充分探索。

Method: 构建了一个受控的多语言自然语言推理评估框架，通过生成合成的逻辑基础前提-假设对，并将其翻译成语类学多样的语言。这种设计允许在单语言和混合语言（语码切换）条件下进行测试。

Result: 语码切换不会降低性能，反而可能提高性能，这表明翻译导致的词汇变化可能作为一种正则化信号。通过嵌入相似性分析和跨语言对齐可视化验证了语义保持的忠实性。

Conclusion: 研究曝露了当前语言模型在跨语言推理方面的潜力和脆弱性，并确定语码切换为改善多语言稳健性的有前景巧套。
...
TL;DR: TransLLM是一个统一的基础框架，通过可学习的提示组合将时空建模与大型语言模型集成，解决了交通领域多任务挑战中现有方法的局限性。
...
Conclusion: TransLLM框架成功整合了时空建模与LLMs，通过动态个性化提示机制实现了跨任务的强适应性和泛化能力，为城市交通系统的多任务挑战提供了有效解决方案。
...
TL;DR: RAG方法在电子病历分析中表现优异，通过检索相关段落显著减少输入token数量，在三个临床任务中接近或超越完整上下文性能


<details>
  <summary>Details</summary>
Motivation: 电子病历冗长、嘈杂且冗余，传统大语言模型受限于上下文窗口长度，需要更高效的解决方案来处理临床文本

Method: 提出三个可复现的临床任务（影像检查提取、抗生素使用时间线生成、关键诊断识别），使用三种先进LLM，比较目标文本检索与最近临床笔记的效果

Result: RAG方法在使用大幅减少输入token的情况下，性能接近或超越使用完整上下文的方法，与使用最近笔记的方法相当或更优

Conclusion: 即使新一代模型能够处理更长文本，RAG仍然是一种竞争性强且高效的方法，特别适合电子病历分析
...
TL;DR: 这篇论文系统研究了长链条思维推理在多语言中的转移效果，发现效果因语言而异，英语作为转换语言的作用有限，多语言预训练稍缩差距但仍需细调，数据质量与规模的权衡依语言而异。


<details>
  <summary>Details</summary>
Motivation: 目前长链条思维推理主要集中在英语上，论文想探索这种推理能力在不同语言间的转移效果，以促进多语言思维推理研究的公平发展。

Method: 构建了两个流行英语推理数据集的翻译版本，对Qwen 2.5 (7B)和Qwen 3 (8B)模型进行细调，系统研究法语、日语、拉脱维亚语和斯瓦希里语中的长链条思维生成。

Result: 三个关键发现：1)英语作为转换语言的效果因语言而异；2)多语言预训练缩小但不能消除跨语言性能差距，轻量细调仍能大幅提升性能；3)数据质量与规模的权衡依语言特性而异。

Conclusion: 研究明确了长链条思维推理在不同语言间转移的条件和原因，为推动多语言思维推理研究的公平发展提供了重要见解和翻译数据集。
...
TL;DR: 提出了MedResearcher医疗深度研究智能体，通过医学知识图谱数据合成和专用检索工具，解决了LLM在医疗领域的知识不足和工具限制问题，在医疗基准测试中达到新SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 通用深度研究智能体在医疗领域表现不佳，主要因为缺乏密集医学知识和专用医疗检索工具，需要开发专门的医疗领域解决方案。

Method: 1) 使用医学知识图谱提取罕见医学实体周围的最长子图链，生成复杂多跳问答对；2) 集成专用医疗检索引擎和通用工具；3) 采用监督微调和在线强化学习的双阶段训练范式。

Result: 生成2100+条多样化轨迹，覆盖12个医学专科，平均每轨迹4.2次工具交互。MedResearcher-R1-32B模型在医疗基准测试中达到新SOTA，同时在通用任务上保持竞争力。

Conclusion: 通过在架构、工具设计和训练数据构建方面的领域特定创新，较小开源模型可以在专业领域超越大型专有系统。
...
TL;DR: 首次系统性研究双向语言模型的量化技术，识别激活值异常值问题并提供多角度评估见解


<details>
  <summary>Details</summary>
Motivation: 双向语言模型在边缘设备上部署面临参数规模大、资源需求高的挑战，而后训练量化技术在这类模型中的应用仍未得到充分探索

Method: 识别激活值异常值问题，应用最新的后训练量化方法，从四个维度（比特宽度、量化方法、任务类型、模型类型）进行全面评估

Result: 提供了不同配置下双向语言模型量化行为的实践见解，为高效部署基础

Conclusion: 研究为双向语言模型的量化压缩提供了重要基础，希望能推动该领域的进一步研究和应用
...
TL;DR: 本研究提出改进的dRRT算法，通过四种策略优化无人机路径规划，在500m³城市环境中相比传统算法表现更优，成功率达100%，平均运行时间0.01468秒。


<details>
  <summary>Details</summary>
Motivation: 随着无人机从'技术工具'向'治理基础设施'转型，低空经济政策和智慧城市需求推动下，高效的路径规划对解决能耗、避障和空域限制等挑战变得至关重要。

Method: 提出增强型快速探索随机树算法(dRRT)，包含四种策略：目标偏向加速收敛、动态步长平衡探索与避障、绕行优先选择水平绕行而非垂直爬升、B样条平滑提升路径平滑度。

Result: 在随机建筑物的城市环境仿真中，dRRT相比传统RRT、A*和蚁群算法表现更优：100%成功率、0.01468s平均运行时间、更短路径、更少航点、更平滑轨迹（最大偏航角<45°）。
...
TL;DR: 这篇论文提出了一种基于强化学习的人形机器人SE(2)目标位式定向行走控制方法，通过算子基奖励函数优化短距离移动的效率和能量消耗。


<details>
  <summary>Details</summary>
Motivation: 现有的学习基础行走方法主要优化速度跟踪，在短距离任务中导致效率低下的行军式行为，无法满足实际工作环境对高速、稳健和能量效率的要求。

Method: 提出了一种新的算子基奖励函数，直接优化人形机器人SE(2)目标位式的行走控制，鼓励自然和高效的目标定向运动。

Result: 在能量消耗、到达时间和脚步数的测量指标上，提出的方法一贵超过标准方法，并成功实现了从模拟到硬件的迁移。

Conclusion: 研究结果高度证明了针对性奖励设计对实际短距离人形机器人行走的重要性，为实际应用提供了有效的解决方案。
...
TL;DR: 基于接触模式的优化方法，统一了人形机器人的移动和操作规划，能够生成长序列物理一致的行为


<details>
  <summary>Details</summary>
Motivation: 解决人形机器人在移动操作任务中的任务、接触和运动规划不统一问题，实现整体动力学和约束考虑的集成规划

Method: 通过将符号动作定义为接触模式变化，使高层规划基于低层运动，并进行统一的任务、接触和运动规划搜索

Result: 在人形机器人平台上能够生成广泛范围的物理一致的移动操作行为，满足长序列复杂推理需求

Conclusion: 这是首个能够解决人形机器人移动操作问题的集成TAMP方法，包含完整无环规划和全身动力学约束
...
TL;DR: 提出基于条件循环生成对抗网络(CCGAN)的领域转换框架，解决软机器人因材料退化导致的跨领域知识迁移问题，实现从标准模拟环境到高粘度环境的控制器适配。


<details>
  <summary>Details</summary>
Motivation: 软机器人材料随时间逐渐退化，传统分析方法需要精确的物理特性知识，而深度学习模型在一个领域学到的映射无法直接迁移到不同物理特性的另一个领域。

Method: 使用条件循环生成对抗网络(CCGAN)，基于输入压力信号和对应的末端执行器位置/方向条件，实现从源领域到目标领域的知识迁移，特别针对粘度增加10倍的环境进行动态学习适配。

Result: 在五种不同形状的轨迹跟踪实验中验证了方法的有效性，并在噪声扰动和周期性测试中展示了鲁棒性，CCGAN-GP成功实现了跨领域技能迁移。

Conclusion: CCGAN-GP框架有效促进了软机器人控制器的跨领域知识迁移，为开发更具适应性和泛化能力的软机器人控制器开辟了新途径。
...
TL;DR: 提出了一种高效可定制的多机器人导航环境，在连续状态动作空间中解决不确定环境下的路径规划问题，通过MDP建模并使用多种RL方法进行验证。


<details>
  <summary>Details</summary>
Motivation: 多机器人在连续状态动作空间中的导航和路径规划在不确定环境下仍是一个开放挑战。深度强化学习虽然流行，但存在样本效率低、训练时间长的问题，且现有方法缺乏形式化保证。

Method: 将多机器人导航任务建模为MDP优化问题，设计了高度可定制的环境，使用A2C、PPO、TRPO、TQC、CrossQ和ARS等梯度/非梯度RL方法进行性能评估，并在CoppeliaSim机器人模拟器中部署到3D农业场景验证鲁棒性。

Result: 开发了适用于真实世界系统的MDP-based环境，能够在有限资源和合理时间内使用现有SOTA RL方法解决问题，并通过模拟器验证了方法的实际适用性。

Conclusion: 该工作为研究人员提供了开发适用于真实世界系统的MDP环境指南，展示了如何使用现有RL方法在资源受限情况下有效解决多机器人导航问题。
...
TL;DR: SimGenHOI是一个结合生成建模和强化学习的统一框架，用于生成可控且物理合理的人形机器人-物体交互动作，解决了现有方法中的接触不合理、穿透和全身动作不真实等问题。
...
Method: 基于Diffusion Transformers的生成模型预测关键动作，结合接触感知的全身控制策略进行强化学习训练，并采用相互微调策略迭代优化生成模型和控制策略。
...
TL;DR: 论文研究了一种基于牛顿-拉弗森流方法的轻量级跟踪控制器，在小型飞艇和中型四旋翼上进行实验验证，相比传统控制方法在保持相近或更好跟踪性能的同时，显著降低了计算时间和能耗。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证这种理论上具有性能保证的轻量级跟踪控制器在真实飞行硬件平台上的实际表现，特别是在面临实际部署约束和机载计算限制的情况下。

Method: 采用基于牛顿-拉弗森流方法的跟踪控制器，与飞艇的反馈线性化和四旋翼/飞艇的非线性模型预测控制等基准方法进行对比实验。

Result: 实验结果显示，牛顿-拉弗森流跟踪控制器在飞行轨迹均方根误差方面达到可比或更优性能，同时计算时间大幅减少，CPU能耗显著降低。

Conclusion: 该轻量级跟踪控制器在真实飞行平台上表现出色，能够在保持良好跟踪性能的同时，大幅降低计算资源消耗，适合在计算受限的机载系统中应用。
...
TL;DR: 提出了一种面向圆形足迹机器人的2D探索策略，使用安全骨架方法在障碍物环境中保持最大安全距离，同时朝向空间开口方向探索未知区域。


<details>
  <summary>Details</summary>
Motivation: 针对传统圆形足迹机器人（而非可瞬时调整位姿的点机器人）在障碍物密集的平面空间中的安全探索需求，优先保证机器人安全避障，其次探索未探测空间。

Method: 采用自平衡双轮差速驱动系统，配备IMU、3D-LiDAR和RGB-D相机。使用IMU和LiDAR进行RTAB-SLAM，相机用于闭环检测。探索时采用安全骨架方法，使机器人尽可能远离静态障碍物，并朝向空间开口方向移动。

Result: 通过ROS移动机器人进行了实验研究，展示了在空间探索过程中的路径规划策略的有效性。

Conclusion: 该方法成功实现了在障碍物环境中安全优先的探索策略，通过传感器融合和安全的路径规划，确保了机器人的安全同时有效探索未知空间。
...
TL;DR: 本研究从蜥蜴等动物的空中翻正反射中提取运动轨迹，通过多目标优化分析关键行为目标，并将其作为空间机械臂系统的参考轨迹进行控制，为空间机器人提供可解释的自适应控制策略。


<details>
  <summary>Details</summary>
Motivation: 空间机械臂系统在微重力环境下的控制面临重大挑战，主要由于机械臂与航天器基座之间的动态耦合。研究探索利用生物灵感来解决这一问题，特别是借鉴具有空中翻正反射能力的动物。

Method: 从高速视频记录中提取蜥蜴等动物的空中翻正运动轨迹，使用计算机视觉技术进行分析。在多目标优化框架下分析这些轨迹，识别关键行为目标并评估其相对重要性。将得到的运动剖面作为参考轨迹应用于空间机械臂系统控制。

Result: 研究成功提取并分析了动物空中翻正运动的轨迹特征，确定了关键行为目标的重要性排序，并将这些运动剖面应用于空间机械臂控制，展示了良好的跟踪性能。

Conclusion: 该方法为将进化动物行为转化为可解释的自适应控制策略提供了重要步骤，对提高未来空间任务的机动性和鲁棒性具有重要意义，为空间机器人控制提供了新的生物启发思路。
...
TL;DR: 提出了一种增强型激光雷达-惯性里程计框架，通过自适应异常值容忍对应和扫描到子地图配准策略，解决LiDAR特征退化问题，在复杂环境中显著提升定位精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: LiDAR-惯性里程计在复杂环境中的精确定位和建图至关重要，但LiDAR特征退化问题严重影响了状态估计的可靠性，需要新的方法来克服这一挑战。

Method: 集成自适应异常值容忍对应与扫描到子地图配准策略，核心是动态调整的异常值去除阈值，基于点传感器距离和平台运动幅度；利用IMU数据优化位姿估计，并设计融合IMU预积分协方差和退化度量的新型权重矩阵。

Result: 在室内外特征稀疏或退化的环境中进行广泛实验，结果表明该方法在鲁棒性和精度方面 consistently优于最先进的方法。

Conclusion: 所提出的增强型LIO框架通过自适应异常值处理和扫描到子地图配准，有效解决了LiDAR特征退化问题，为复杂环境下的可靠状态估计提供了有效解决方案。
...
TL;DR: 提出DTWIL方法解决动作受限模仿学习问题，通过轨迹对齐和动态时间规整距离生成替代数据集，在多个机器人控制任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 在机器人控制和资源分配应用中，动作约束下的策略学习对于确保安全行为至关重要。研究动作受限模仿学习问题，其中模仿者需要从动作空间更大的专家演示中学习

Method: 提出DTWIL方法，通过轨迹对齐将专家演示替换为遵循相似状态轨迹但遵守动作约束的替代数据集。将轨迹对齐重新表述为规划问题，使用模型预测控制基于动态时间规整距离对齐轨迹

Result: 实验表明，从DTWIL生成的数据集学习显著提高了多个机器人控制任务的性能，在样本效率方面优于各种基准模仿学习算法
...
<details>
  <summary>Details</summary>
Motivation: 当前UAM飞行规划器可能导致某些无人机获得不成比例的短路径，而牺牲其他无人机的利益，这会影响运营商参与UAM的积极性。需要一种公平的路径规划方法来促进更广泛的参与和确保空域安全。

Method: Fair-CoPlan采用三步骤方法：1)服务提供商基于垂直起降场容量约束起降选择；2)运营商在约束下独立规划；3)服务提供商解决路径冲突，优化路径长度公平性。通过公平分摊解冲突成本来鼓励参与。

Result: 仿真实验表明，Fair-CoPlan相比非公平规划器能产生更公平的结果，虽然会带来轻微的延迟作为权衡，但实现了更好的公平性分配。

Conclusion: Fair-CoPlan通过公平的飞行路径规划促进了UAM的更广泛参与，确保了空域及其下方区域的安全，并为运营商提供了更大的灵活性，在公平性和效率之间取得了良好平衡。
...
TL;DR: 提出FiReFly算法，将公平性概念注入多机器人运动规划，在保证任务成功的同时公平分配机器人能耗，在模拟实验中相比非公平规划器产生更公平轨迹并提高任务成功率


<details>
  <summary>Details</summary>
Motivation: 当机器人存在竞争利益时，需要优化资源使用的公平性分配，特别是机器人的能量消耗公平分布问题

Method: 制定分布式公平运动规划器，并与安全控制器集成形成FiReFly算法，用于可达-避障任务

Result: 在模拟实验中，FiReFly产生更公平的轨迹，提高任务成功率，实时性能可达15架无人机，扩展到50架需在运行时间和公平性改进之间权衡

Conclusion: FiReFly算法成功将公平性整合到多机器人运动规划中，在保持任务成功的同时实现能量消耗的公平分配，具有良好的可扩展性
...
TL;DR: 通过引入动力学表征预训练阶段，从任意数据中学习转移动力学的因子化表示，显著提升有限专家数据下的离线模仿学习性能，仅需单条轨迹即可模仿专家策略
...
Method: 提出预训练阶段学习动力学表征（转移动力学的因子化表示），使用噪声对比估计的损失函数，可从任意同动力学数据中学习，重用大量非专家数据

Result: 在MuJoCo上仅需单条轨迹即可模仿专家策略，在真实四足机器人上可利用模拟器预训练的动力学表征从少量真实演示中学习行走

Conclusion: 动力学表征预训练有效缓解了离线模仿学习中的数据限制问题，实现了从模拟到真实世界的知识迁移
...
TL;DR: DEXTER-LLM是一个用于未知环境中动态任务规划的框架，结合LLM的开放世界推理能力和基于模型的优化方法，解决了在线适应性和可解释性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的场景推理和规划方法主要针对已知环境的一次性端到端解决方案，缺乏在线操作的动态适应能力和规划过程的可解释性。
...
Conclusion: 该框架成功结合了LLM的开放世界推理能力和模型化分配方法的最优性，同时解决了在线适应性和可解释性关键问题。
...
TL;DR: FBI是一个视觉触觉模仿学习框架，通过运动动力学动态融合触觉交互和视觉观察，用于灵巧手内操作任务


<details>
  <summary>Details</summary>
Motivation: 灵巧手内操作由于复杂的接触动力学和部分可观测性而具有挑战性。人类能够协同使用视觉和触觉，但机器人方法通常只优先考虑一种模态，限制了适应性

Method: FBI通过动态感知潜在模型建立触觉信号与物体运动之间的因果关系，使用基于transformer的交互模块融合流动衍生的触觉特征和视觉输入，训练一步扩散策略进行实时执行

Result: 在仿真和真实世界的两个定制手内操作任务和三个标准灵巧操作任务上，该方法优于基线方法
...
TL;DR: ICRA 2025 WBCD竞赛冠军解决方案，结合VR遥操作和学习演示技术，成功完成桌布展开、披萨放置和食品容器开关等复杂双手机器人任务。


<details>
  <summary>Details</summary>
Motivation: 解决双手机器人在速度、精度和可靠性严格要求下的复杂桌面服务任务，包括可变形物体操作、精准抓取放置等挑战。

Method: 采用VR遥操作与学习演示(LfD)相结合的方法，大部分子任务通过高保真远程遥操作执行，披萨放置任务使用基于ACT策略的100次现场遥操作演示训练。

Result: 通过精心整合评分规则、任务特性和当前技术能力，实现了高效率和高可靠性，最终获得竞赛第一名。

Conclusion: VR遥操作与学习演示的混合方法在复杂双手机器人任务中表现出色，为实际应用提供了可靠的技术方案。
...
TL;DR: 提出了EAROL框架，通过向下倾斜20度的LiDAR配置，结合LiDAR-惯性里程计和分层轨迹-偏航优化算法，解决了无人机在开放顶部场景中的定位漂移和感知-规划耦合问题。
...
Method: 采用向下倾斜20度的LiDAR硬件配置，结合紧密耦合的LiDAR-惯性里程计系统（使用迭代误差状态卡尔曼滤波和动态运动补偿）以及分层轨迹-偏航优化算法。
...
TL;DR: TRUST-Planner是一个拓扑引导的层次化规划框架，通过动态增强可见概率路线图(DEV-PRM)和统一无终端最小控制多项式(UTF-MINCO)等技术，解决了自主飞行器在复杂动态环境中的局部最小值和死锁问题，实现了96%的成功率和毫秒级计算效率。
...
Method: 采用拓扑引导的层次化规划框架：前端使用动态增强可见概率路线图(DEV-PRM)快速探索拓扑路径；后端使用统一无终端最小控制多项式(UTF-MINCO)和动态距离场(DDF)实现预测性避障和并行计算；引入增量多分支轨迹管理框架进行时空拓扑决策。

Result: 仿真结果显示TRUST-Planner优于基线方法，在复杂环境中达到96%的成功率和毫秒级计算效率，真实世界实验验证了方法的可行性和实用性。

Conclusion: TRUST-Planner通过拓扑引导的层次化规划有效解决了自主飞行器在动态环境中的运动规划问题，具有高成功率和实时计算性能，为复杂环境下的安全导航提供了有效解决方案。
...
TL;DR: 本研究探讨LLM智能体在多智能体协作救援任务中的表现，通过系统评估其在分工、优先级排序和合作规划方面的能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在沟通、规划和推理方面展现出强大能力，但需要验证其是否能在多智能体环境中支持有效协作，特别是在需要物理基础协作的复杂任务中。

Method: 使用LLM智能体在完全已知的图基环境中执行结构化受害者救援任务，评估其资源分配、分工和合作规划能力，采用协调敏感指标进行系统评估。

Result: 研究提供了关于LLM在物理基础多智能体协作任务中优势和失败模式的新见解，包括任务成功率、冗余行动、房间冲突和紧急加权效率等指标。

Conclusion: 该研究为未来基准测试和架构改进提供了重要参考，有助于理解LLM在多智能体协作中的潜力和局限性。
...
<details>
  <summary>Details</summary>
Motivation: 移动机器人在动态环境中高效跟踪漂浮目标（如受风浪影响的污染物）对搜救和环保清理等应用至关重要，需要解决时空变化的预测挑战

Method: 开发了包含时空预测网络的信息路径规划框架，能够预测目标位置随时间分布，并提出自适应规划目标来优化目标跟踪

Result: 仿真实验表明，相比仅考虑熵减少的现有方法，提出的规划目标显著提升了目标跟踪性能。实地测试使用自主水面车辆验证了在真实监测场景中的跟踪能力

Conclusion: 该框架成功解决了动态环境中移动目标的主动映射问题，时空预测和自适应规划目标的结合有效提升了跟踪性能，具有实际应用价值
...
<details>
  <summary>Details</summary>
Motivation: 增强扩展卡尔曼滤波器在地面车辆定位中的一致性和长期精度，避免因人工约束或简化导致的性能下降。

Method: 使用流形误差状态扩展卡尔曼滤波器(M-ESEKF)，在降维空间中表示位姿，开发兼容松紧耦合传感器模式的测量模型，并引入新颖的校正方案嵌入领域知识。

Result: 广泛的蒙特卡洛模拟显示，M-ESEKF在一致性和稳定性方面优于经典滤波器，无需场景特定的参数调优，适用于各种实际场景。
...
TL;DR: 开发了一个用于肉类加工机器人的安全和透明度框架，包括手部检测系统和仪器化刀具确保安全，以及LED界面和图形界面提供透明度


<details>
  <summary>Details</summary>
Motivation: 肉类加工行业面临劳动力短缺，现有自动化设备过于专业化、不灵活且成本高昂，需要开发能与人类协作的通用机器人系统

Method: 实现手部检测系统监控附近人类安全；开发带力传感器的仪器化刀具区分接触对象；设计LED界面显示机器人不确定性；创建图形界面展示机器人计划并允许人类反馈

Result: 通过用户研究验证了该框架能够确保安全操作，同时让人类工人了解机器人的行动

Conclusion: 提出的安全和透明度框架可以有效解决肉类加工机器人集成中的两大挑战，为通用机器人系统与人类协作提供了可行方案
...
### [139] [A Real-world Display Inverse Rendering Dataset](https://arxiv.org/abs/2508.14411)
*Seokjun Choi,Hoon-Gyu Chung,Yujin Jeon,Giljoo Nam,Seung-Hwan Baek*

Main category: cs.GR

TL;DR: 首个基于显示器-相机系统的真实世界逆渲染数据集，包含LCD显示器和立体偏振相机采集的OLAT图像，提供高质量几何真值，支持任意显示模式和噪声水平的图像合成。


<details>
  <summary>Details</summary>
Motivation: 显示器-相机成像系统具有独特优势（像素可编程光源、LCD偏振光便于漫反射-镜面反射分离），但缺乏公开的真实世界数据集阻碍了基于显示的逆渲染方法发展。

Method: 构建并校准LCD显示器和立体偏振相机成像系统，采集多样化几何和反射率物体在OLAT显示模式下的图像，提供高质量几何真值。

Result: 数据集支持任意显示模式和不同噪声水平的图像合成，评估显示现有方法性能，并提出简单有效的基线方法，优于最先进的逆渲染方法。

Conclusion: 该数据集填补了显示基逆渲染领域的空白，为方法开发和评估提供了重要资源，提出的基线方法展现了优越性能。
...
### [140] [MeshCoder: LLM-Powered Structured Mesh Code Generation from Point Clouds](https://arxiv.org/abs/2508.14879)
...
TL;DR: MeshCoder是一个将3D点云重建为可编辑Blender Python脚本的新框架，通过多模态大语言模型实现复杂几何结构的程序化重建和编辑。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖有限的领域特定语言和小规模数据集，难以处理复杂几何结构和形状编辑需求。

Method: 开发了一套全面的Blender Python API来合成复杂几何体，构建大规模配对的物体-代码数据集，并训练多模态大语言模型将3D点云转换为可执行脚本。

Result: 在形状到代码重建任务中取得优异性能，支持通过代码修改进行直观的几何和拓扑编辑，并增强了LLM在3D形状理解任务中的推理能力。

Conclusion: MeshCoder为程序化3D形状重建和理解提供了一个强大而灵活的解决方案。
...
### [141] [Snap-Snap: Taking Two Images to Reconstruct 3D Human Gaussians in Milliseconds](https://arxiv.org/abs/2508.14892)
*Jia Lu,Taoran Yi,Jiemin Fang,Chen Yang,Chuiyun Wu,Wei Shen,Wenyu Liu,Qi Tian,Xinggang Wang*

Main category: cs.GR

TL;DR: 提出从仅两张图像（正面和背面）重建3D人体的方法，通过几何重建模型和增强算法实现快速高质量的完整人体重建。


<details>
  <summary>Details</summary>
Motivation: 从稀疏视图重建3D人体是重要应用，但现有方法需要多视角图像。本文旨在降低用户创建3D数字人的门槛，仅需两张图像即可完成重建。

Method: 基于基础重建模型重新设计几何重建模型来预测一致的点云，即使输入图像重叠很少；应用增强算法补充缺失的颜色信息；将完整点云转换为3D高斯分布以获得更好的渲染质量。

Result: 在单张NVIDIA RTX 4090上仅需190ms即可重建完整人体，输入为1024x1024分辨率的两张图像，在THuman2.0和跨域数据集上达到最先进性能，且支持低成本移动设备拍摄的图像。

Conclusion: 该方法显著降低了3D人体重建的数据采集要求，仅需两张图像即可快速生成高质量的3D数字人模型，具有很好的实用性和推广价值。
...
### [142] [Large Language Models are Highly Aligned with Human Ratings of Emotional Stimuli](https://arxiv.org/abs/2508.14214)
*Mattson Ogg,Chace Ashcraft,Ritwik Bose,Raphael Norman-Tenazas,Michael Wolmetz*

Main category: cs.AI

TL;DR: GPT-4o在情绪评估任务中与人类表现高度相似，但在唤醒度评分上存在差异，整体在五分类情绪框架中比二维情绪维度模型更接近人类评分


<details>
  <summary>Details</summary>
Motivation: 了解大型语言模型如何评估情绪刺激，为LLM在日常生活中的应用提供依据，特别是在需要情绪理解的角色和交互中

Method: 使用人类先前评定的情绪词汇和图像数据集，让多个流行LLM进行相同的评分任务，比较LLM与人类在情绪评估上的一致性

Result: GPT-4o在大多数评分维度上与人类参与者非常相似（r≥0.9），但唤醒度评分一致性较差，快乐评分一致性最高。LLM在五分类情绪框架中比二维情绪维度模型中更接近人类，且LLM评分比人类评分更加同质化

Conclusion: 研究揭示了LLM代理如何解释情绪刺激，突出了生物智能和人工智能在关键行为领域的相似性和差异，为LLM在需要情绪理解的应用中提供了重要参考
...
### [143] [Explaining Hitori Puzzles: Neurosymbolic Proof Staging for Sequential Decisions](https://arxiv.org/abs/2508.14294)
...
<details>
  <summary>Details</summary>
Motivation: Hitori谜题包含局部约束和连通性约束，需要不同类型的解释方法。局部约束适合用简短的解析证明解释，而连通性约束更适合视觉解释，这为结合符号推理和神经语言模型提供了理想的测试场景。

Method: 开发了一个神经符号方法工具，结合SAT求解器（处理局部约束的解析证明）和大型语言模型（生成连通性约束的视觉解释），辅助人类解决Hitori谜题。

Result: 实现了有效的工具，实验证据表明该方法在解释Hitori谜题解决方案方面具有有效性，能够灵活处理不同类型的约束条件。

Conclusion: 神经符号方法成功结合了符号推理和语言模型的优势，为复杂决策序列的解释提供了有效框架，特别适用于包含多种约束类型的推理问题。
...
### [144] [Automated Optimization Modeling through Expert-Guided Large Language Model Reasoning](https://arxiv.org/abs/2508.14410)
*Beinuo Yang,Qishen Zhou,Junyi Li,Xingchen Su,Simon Hu*

Main category: cs.AI

TL;DR: ORThought框架通过思维链推理自动化优化建模，在复杂问题上显著优于现有方法，解决了基准标注错误率高、评估范围窄和计算效率低三大挑战。


<details>
  <summary>Details</summary>
Motivation: 优化建模过程耗时且易出错，严重依赖领域专家。虽然大语言模型展现潜力，但现有方法存在基准标注错误率高（达42%）、评估范围窄（仅考虑最优值）和计算效率低（依赖多智能体系统或微调）三大局限。

Method: 提出ORThought框架，利用专家级优化建模原则通过思维链推理自动化优化建模过程。首先增强现有数据集，系统纠错并扩展标注；引入LogiOR物流领域新基准，包含更复杂问题的标准化标注。

Result: ORThought在广泛实证评估中优于现有方法（包括多智能体框架），在复杂优化问题上优势尤为显著。提供了系统分析方法，识别关键成功因素和失败模式。

Conclusion: 该工作为基于LLM的优化建模提供了有价值的见解，通过改进数据集质量、引入更复杂基准和创新推理框架，显著提升了自动化优化建模的性能和可靠性。
...
### [145] [The Agent Behavior: Model, Governance and Challenges in the AI Digital Age](https://arxiv.org/abs/2508.14415)
*Qiang Zhang,Pei Yan,Yijia Xu,Chuanpo Fu,Yong Fang,Yang Liu*

Main category: cs.AI

TL;DR: 本文提出了网络行为生命周期模型和A4A范式，通过6阶段分析和5维度差异模型来区分人类与AI代理行为，为解决网络环境中人机协作的信任和安全问题提供理论框架。


<details>
  <summary>Details</summary>
Motivation: AI代理在网络环境中越来越像人类行为，导致信任、责任、伦理和安全等方面的挑战，需要系统化区分人类与代理行为以解决监管难题。

Method: 提出网络行为生命周期模型（6阶段分析）、A4A范式和HABD模型（5维度差异分析：决策机制、执行效率、意图行为一致性、行为惯性和非理性模式），并通过红蓝队攻防实战验证。

Result: 模型有效区分了人类与代理行为差异，为安全可信的人机协作提供了理论基础，验证了在真实网络攻防场景中的实用性。

Conclusion: 该研究为人机协作安全提供了重要理论框架，未来需要发展动态认知治理架构、行为差异量化和元治理协议栈等方向，推动安全可信的人机协同发展。
...
### [146] [Who Sees What? Structured Thought-Action Sequences for Epistemic Reasoning in LLMs](https://arxiv.org/abs/2508.14564)
*Luca Annese,Sabrina Patania,Silvia Serino,Tom Foulsham,Silvia Rossi,Azzurra Ruggeri,Dimitri Ognibene*

Main category: cs.AI

TL;DR: 本研究探讨了使用规划器生成的转换解图来创建结构化示例，以提升基于LLM的智能体在视角采择任务中的表现，但发现仅靠结构化示例不足以实现稳健的视角采择能力。
...
Method: 提出结构化解处理流水线，生成三类示例：最优目标路径(G型)、信息节点路径(E型)和对比替代行动的最优决策序列(L型)，并通过LLM转换为"思考-行动"示例。
...
### [147] [LeanGeo: Formalizing Competitional Geometry problems in Lean](https://arxiv.org/abs/2508.14644)
*Chendong Song,Zihan Wang,Frederick Pu,Haiming Wang,Xiaohan Lin,Junqi Liu,Jia Li,Zhengying Liu*

Main category: cs.AI

TL;DR: LeanGeo是一个基于Lean 4定理证明器的统一几何形式化系统，用于解决竞赛级几何问题，包含几何定理库和LeanGeo-Bench基准测试


<details>
  <summary>Details</summary>
Motivation: 现有几何求解系统难以在统一框架下表达问题，且几何证明依赖直观图表，验证困难

Method: 在Lean 4中构建综合几何定理库，利用Lean的基础逻辑进行严格证明验证，并与Mathlib无缝集成

Result: 创建了包含IMO等高级来源问题的LeanGeo-Bench基准，评估了大型语言模型在几何推理方面的能力与局限

Conclusion: 该系统展示了自动化几何推理的潜力，但需要进一步的技术进步，相关资源已开源
...
### [148] [Entropy-Constrained Strategy Optimization in Urban Floods: A Multi-Agent Framework with LLM and Knowledge Graph Integration](https://arxiv.org/abs/2508.14654)
...
<details>
  <summary>Details</summary>
Motivation: 极端城市降雨事件频发导致严重交通拥堵和服务中断，现有方法在管理竞争目标权衡、应对快速变化环境条件以及确保LLM生成策略的稳定性方面存在不足，需要统一的感知、全局优化和多智能体协调框架。
...
### [149] [MCP-Universe: Benchmarking Large Language Models with Real-World Model Context Protocol Servers](https://arxiv.org/abs/2508.14704)
*Ziyang Luo,Zhiqi Shen,Wenzhuo Yang,Zirui Zhao,Prathyusha Jwalapuram,Amrita Saha,Doyen Sahoo,Silvio Savarese,Caiming Xiong,Junnan Li*

Main category: cs.AI

TL;DR: MCP-Universe是首个专门为评估LLM与真实MCP服务器交互能力而设计的综合基准测试，涵盖6个核心领域11个服务器，发现即使是SOTA模型在长上下文和未知工具方面仍存在显著性能限制。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试过于简单，无法捕捉真实应用中的长时程推理和大规模陌生工具空间等挑战，需要更全面的评估标准。

Method: 构建包含6个核心领域11个MCP服务器的基准测试，采用执行式评估器（格式评估器、静态评估器、动态评估器）进行严格评估。

Result: 主流SOTA模型表现不佳（GPT-5:43.72%、Grok-4:33.33%、Claude-4.0-Sonnet:29.44%），存在长上下文和未知工具使用挑战，企业级代理性能不优于标准ReAct框架。

Conclusion: MCP-Universe填补了重要空白，为MCP生态系统提供了可扩展的开源评估框架，促进该领域的创新和发展。
...
### [150] [Data-Driven Probabilistic Evaluation of Logic Properties with PAC-Confidence on Mealy Machines](https://arxiv.org/abs/2508.14710)
*Swantje Plambeck,Ali Salamati,Eyke Huellermeier,Goerschwin Fey*

Main category: cs.AI

TL;DR: 基于PAC学习框架的数据驱动方法，通过主动学习范式估算CPS系统在有限时间范围内的安全概率，并提供概率置信度
...
Method: 采用基于Mealy机离散抽象的主动学习范式，在初始数据集基础上通过引导采样收集新数据，基于PAC学习框架估算安全概率

Result: 方法在自动车道保持系统案例研究中得到验证，能够有效确定系统在有限时间步长内的安全概率

Conclusion: 该方法建立了离散逻辑与概率可达性分析之间的联系，为CPS系统的安全概率评估提供了具有置信度的数据驱动解决方案
...
TL;DR: 论文探讨AI模型是否能内省，提出了比现有"轻量级"定义更"厚重"的内省定义，并通过LLM温度参数推理实验证明模型可能表现出轻量级内省但缺乏真正的内省能力。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型发展，判断其是否具备内省能力成为重要实践问题，但目前缺乏统一的内省定义标准。

Method: 提出新的内省定义：任何能通过比第三方更可靠且计算成本相当或更低的过程获取内部状态信息的过程。通过LLM推理自身温度参数的实验进行验证。

Result: 实验显示LLM可能表现出轻量级内省的表象，但按照作者提出的更严格定义，它们实际上缺乏有意义的内省能力。

Conclusion: 需要更严格的内省定义标准来准确评估AI模型的内省能力，当前LLM的内省表现可能只是表面现象而非真正的内省。
